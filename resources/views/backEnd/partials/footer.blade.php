...
<script>
  window.jsLang = window.jsLang || function (key, replace) {
    let translation = key; // Default to key if no translation found

    if (window._translations && window._translations[window._locale]) {
        let json_file = window._translations[window._locale]['json'];
        if (json_file) {
            let parsed_json = JSON.parse(json_file);
            translation = parsed_json[key] ? parsed_json[key] : key;
        }
    }
...
<script type="text/javascript">
    $(document).ready(function() {
        $('#school_date_from').datepicker();
        $('#school_date_to').datepicker();
    });
</script>
@yield('script')
@stack('script')
