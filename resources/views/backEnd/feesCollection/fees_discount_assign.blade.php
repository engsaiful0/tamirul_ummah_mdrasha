...
@section('mainContent')
    <section class="admin-visitor-area">
        <div class="container-fluid">
                    ...
                    <div class="white-box">
                        ...
                        <div class="row">
                            ...
                            @if(moduleStatusCheck('University'))
                                ...
                                @else
                                <div class="col-md-4 col-lg-4 mt-30-md">
                                ...
                                <div class="col-md-4 col-lg-4 mt-30-md" id="select_section_div">
            ...
            @if(!empty($students))
                {{ Form::open(['class' => 'form-horizontal', 'id' => 'fees_discount_assign_store', 'method' => 'POST', 'url' => 'fees-discount-assign-store','onsubmit' => "return validateFeeType()"])}}
                    <div class="row mt-20">
                        <div class="col-lg-12">
                            <div class="row">
                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                    <x-table>
                                        <table id="table_id_table1" class="table" cellspacing="0" width="100%">
                                            <thead>
                                                <tr>
                                                    <tr>
                                                        <th width="20%">@lang('fees.fees_discount')</th>
                                                    ...
                                                    @foreach($students as $student)
                                                        <tr id="student_row.{{$student->id}}">
                                                            <td>
                                                                ...
                                                                @endphp
                                                                <input type="checkbox" id="student.{{$student->id}}" {{@$show}} class="common-checkbox student-selected" name="data[{{$loop->index}}][checked]" value="1" {{in_array($student->student_id, $pre_assigned)? 'checked':''}} >
                                                            ...
                                                            <td>
                                                                ...
                                                                @if($fees_discount->type=='once')
                                                                    @if($check_discount_apply=='')
                                                                        <select class="primary_select w-100  form-control{{ $errors->has('fees_master_id') ? ' is-invalid' : '' }} select_fees_master student_fee_type.{{$student->id}}" {{@$show}} name="data[{{$loop->index}}][fees_master_id]" id="fees_master{{$student->id}}">
                                                                            ...
                                                                            @foreach($assigned_fees_groups[$student->id] as $key=> $fees_group)
                                                                                ...
                                                                                @endphp
                                                                                    ...
                                                                                    @foreach ($studentAssingFees_types as $fees_type)
                                                                                        <option value="{{$fees_type->fees_type_id}}">{{$fees_type->name}}</option>
                                                                                    @endforeach
                                                                            @endforeach
                                                                        </select>
                                                                        <span class="fees-error-msg text-danger err_fee_type.{{$student->id}}"></span>
                                                                        ...
                                                                        @endif
                                                                    @else      
                                                                        <select class="primary_select w-100  form-control{{ $errors->has('fees_master_id') ? ' is-invalid' : '' }} select_fees_master student_fee_type.{{$student->id}}" name="data[{{$loop->index}}][fees_master_id]" title="Discount Applied" id="fees_master{{$student->id}}">
                                                                            <option data-display="@lang('fees.select_fees_type') *" value=""> @lang('fees.select_fees_type') *</option>
                                                                        ...
                                                                        <span class="text-danger err_fee_type.{{$student->id}}"></span>
                                                                        @if ($errors->has('fees_master_id'))
                                                                            <span class="text-danger invalid-select" role="alert">
                                                                                {{ $errors->first('fees_master_id') }}
                                                                            </span>
                                                                ...
                                                                @else
                                                                    ...
                                                                    @endphp
                                                                    @if ($check_discount_apply=='')
                                                                        <select class="primary_select w-100  form-control{{ $errors->has('fees_master_id') ? ' is-invalid' : '' }} select_fees_master student_fee_type.{{$student->id}}" name="data[{{$loop->index}}][fees_master_id]" {{@$show}} id="fees_master{{$student->id}}">
                                                                    ...
                                                                    @else
                                                                        ...
                                                                        <select class="primary_select w-100  form-control{{ $errors->has('fees_master_id') ? ' is-invalid' : '' }} select_fees_master student_fee_type.{{$student->id}}" name="data[{{$loop->index}}][fees_master_id]]" id="fees_master{{$student->id}}">
...
$(document).ready(function() { 
    $(document).on('submit', '#fees_discount_assign_store', function() {
        var student_checked = 0;
        var fees_type_error = false; 
        
        $('.student-selected:checked').each(function() {
            student_checked++; 
            var rowId = $(this).attr('id').split('.')[1]; 
            var selectedFeesType = $('#fees_master' + rowId).val();
            if (!selectedFeesType) {
                fees_type_error = true;
                $('.err_fee_type.' + rowId).text("Fees type is required");
            } else {
                $('.err_fee_type.' + rowId).text(""); 
            }
        });

        if (student_checked == 0) {
            setTimeout(function() {
                toastr.error("At least one student must be selected", "Error Alert", { timeOut: 5000 });
            }, 500);
            return false;
        }
        ...
        if (fees_type_error) {
            setTimeout(function() {
                toastr.error("Please select fees type for all selected students", "Error Alert", { timeOut: 5000 });
            }, 500);
            return false;
        }
        return true;
