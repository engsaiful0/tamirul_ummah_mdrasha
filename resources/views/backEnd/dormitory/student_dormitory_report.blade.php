                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if(isset($class_id) && $class_id != "")
                                                    @php
                                                        $currentRecord = $student->studentRecords->where('class_id', $class_id)->first();
                                                        if($currentRecord) {
                                                            echo $currentRecord->class->class_name ?? '';
                                                        }
                                                    @endphp
                                                    @if(isset($section_id) && $section_id != "")
                                                        @php
                                                            $sectionRecord = $student->studentRecords->where('class_id', $class_id)->where('section_id', $section_id)->first();
                                                        @endphp
                                                        {{ $sectionRecord && $sectionRecord->section ? $sectionRecord->section->section_name : '' }}
                                                    @else
                                                        @php
                                                            $classRecords = $student->studentRecords->where('class_id', $class_id);
                                                        @endphp
                                                        (@foreach ($classRecords as $record)
                                                            {{ $record->section ? $record->section->section_name : '' }}{{ !$loop->last ? ', ' : '' }}
                                                        @endforeach)
                                                    @endif
                                                @else
                                                    @foreach ($student->studentRecords as $record)
                                                        {{__('common.class') }} : {{ $record->class ? $record->class->class_name : '' }}({{ $record->section ? $record->section->section_name : '' }}){{ !$loop->last ? ', ' : '' }} <br>
                                                    @endforeach
                                                @endif
                                            ...
                                            <td>{{@$student->parents && $student->parents->guardians_mobile ? $student->parents->guardians_mobile : '-'}}</td>
                                            <td>{{ $student->dormitory && $student->dormitory->dormitory_name ? $student->dormitory->dormitory_name : '-'}}</td>
                                            <td>{{ $student->room && $student->room->name ? $student->room->name : '-'}}</td>
                                            <td>{{ $student->room && $student->room->roomType && $student->room->roomType->type ? $student->room->roomType->type : '-'}}</td>
                                            <td>{{ $student->room && $student->room->cost_per_bed ? number_format( (float) $student->room->cost_per_bed, 2, '.', '') : '-'}}</td>
                                        </tr>
                                        @endforeach
                                        @else
                                        <tr>
                                            <td colspan="{{ moduleStatusCheck('University') ? '8' : '9' }}" class="text-center">@lang('common.no_data_available')</td>
                                        </tr>
                                        @endif
                                    </tbody>
                                </table>
