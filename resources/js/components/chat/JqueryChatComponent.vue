...
<vue-editor v-model="newMessage" :editor-toolbar="customToolbar" ref="contenteditor"/>
                            ...
                            <div class="input-group-append">
                                <button class="btn pr-2" type="button" @click="toggleRecording" :title="__('voice_recording')" :style="{ color: record_status ? '#e74c3c' : '#989898' }">
                                    Record
                                </button>
                                <button class="btn" type="button" @click="emoji = !emoji" :title="__('emoji')" :style="{ color: emoji ? '#4CAF50' : '#989898' }">
                                    Emoji
                                </button>
                                <button v-if="can_file_upload" class="btn" type="button" :title="__('attach_file')" style="position: relative; color: #989898;">
                                    File Upload
                                    <input type="file" @change="onFileChange" id="imgInp" ref="file" v-on:change="onChangeFileUpload()" accept=".jpg,.jpeg,.png,.doc,.docx,.pdf,.mp4,.3gp,.webm" style="position:absolute;top:0;left:0;width:100%;height:100%;opacity:0;cursor:pointer;">
                                </button>
...
.chat_view_list .chat_input_box .input-group-append .btn:hover {
    background-color: rgba(0,0,0,0.05);
}
.chat_view_list .chat_input_box .input-group-append .btn svg {
    display: block;
}
