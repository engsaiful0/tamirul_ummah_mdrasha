...
if (!function_exists('userPermission')) {
    function userPermission($route, $role_id = null, $purpose = null)
    {
        ...
        // If numeric ID is passed, convert it to route name
        if (is_numeric($route)) {
            $permission = \Modules\RolePermission\Entities\Permission::where('old_id', $route)->first();
            if ($permission && $permission->route) {
                $route = $permission->route;
            } else {
                // If old_id not found, try finding by id
                $permission = \Modules\RolePermission\Entities\Permission::find($route);
                if ($permission && $permission->route) {
                    $route = $permission->route;
                } else {
                    return false;
...
if (!function_exists('markResult')) {
    function markResult($marks)
    {
        $mark = SmMarksGrade::where([
            ['percent_from', '<=', floor($marks)],
            ['percent_upto', '>=', floor($marks)]
    ...
    function send_sms($reciver_number, $purpose, $data)
    {
        ...
        try {
            if ($activeSmsGateway->gateway_name == 'Twilio') {
                ...
                $client = new Twilio\Rest\Client($account_id, $auth_token);
                $result = $client->messages->create($reciver_number, array('from' => $from_phone_number, 'body' => $body));
            } else if ($activeSmsGateway->gateway_name == 'Msg91') {
                $msg91_authentication_key_sid = $activeSmsGateway->msg91_authentication_key_sid;
                $msg91_sender_id = $activeSmsGateway->msg91_sender_id;
                $msg91_route = $activeSmsGateway->msg91_route;
...
if (!function_exists('isSignUpAllowed')) {
...
if (!function_exists('sendSMSBioWithSchoolId')) {
function sendSMSBioWithSchoolId($to_mobile, $sms, $schoolId)
{
