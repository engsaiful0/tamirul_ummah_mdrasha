...
if(!function_exists('currency_format')) {
    function currency_format($amount = null, string $format = null)
    {
        ...
        $symbolCode = $format->currency_type == 'C' ? $format->code : generalSetting()->currency_symbol;
...
if(!function_exists('storePermissionData')) {
    ...
    {
        // For permission sections, first check if they exist by route (regardless of user_id)
        // This handles cases where they might exist with null user_id or different user_id
        if($permission['permission_section'] == 1 && $user_id) {
            $existingPermission = Permission::where('route', $permission['route'])
            ...
            if($existingPermission) {
                // Update existing permission with correct user_id
            ...
            // For non-permission sections, use the original updateOrCreate logic
        ...
        if(isset($permission['child'])) {
            foreach($permission['child'] as $child) {              
                storePermissionData($child, $user_id, $school_id);
            }
        }
    }
}
...
//Menu Realignment
if(!function_exists('sidebar_menus_new')) {
    function sidebar_menus_new($parent_route)
    {
        $user = auth()->user();
        if($user->role_id==3){
            $query = Sidebar::with(['subModule', 'permissionInfo'])
                ->whereHas('permissionInfo', function ($q) use ($user, $parent_route) {
                    $q->where('menu_status', 1)
                        ->where('route', $parent_route)
                        ->when($user->role_id == 2, function ($q) {
                            $q->where('is_student', 1);
                        })->when($user->role_id == 3, function ($q) {
                            $q->where('is_parent', 1);
            ...
            $query = Sidebar::with(['subModule', 'permissionInfo'])
                ->whereHas('permissionInfo', function ($q) use ($user, $parent_route) {
                    $q->where('menu_status', 1)
                        ...
                        ->orWhere('sidebar_menu', $parent_route)
                        ->whereNotNull('parent_route')
                        ->when($user->role_id == 2, function ($q) {
                            $q->where('is_student', 1);
                        })->when($user->role_id == 3, function ($q) {
                            $q->where('is_parent', 1);
