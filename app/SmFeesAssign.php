...
class SmFeesAssign extends Model
{
    ...
    public static function transportRouteFees($student_id)
    {
        try {
            $fees = SmRoute::select('sm_routes.id','sm_routes.far','sm_routes.title')
                ->leftjoin('sm_students','sm_students.route_list_id','=','sm_routes.id')
    ...
    public static function transportFeesPayment($month_id, $student_id, $record_id){
        try {
            $payments = SmTransportFeesPayment::where('active_status',1)
    ...
    public static function transportPaymentSum($month_id, $student_id, $perpose, $record_id)
    {
        try {
            $sum = SmTransportFeesPayment::where('active_status',1)
                 ->where('student_id', $student_id)
                 ->where('record_id', $record_id)
                 ->where('month_id', $month_id)
                ->sum($perpose);
    ...
    public static function transportRouteFeesPaid($month_id,$student_id,$record_id)
    {
        try {

            $paidInfo = SmTransportFeesPayment::where('student_id', $student_id)
                 ->where('record_id', $record_id)
                 ->where('month_id', $month_id)
                 ->first();
            return $paidInfo;
    ...
    public static function paidTransportRoute($route_id)
    {
        try {
            $paidRoute = SmRoute::where('id', $route_id)->first();
            return $paidRoute;
        } catch (\Exception $e) {
            $data=[];
            return $data;
    ...
    public static function feesPaymentBy($user_id){
        try {
