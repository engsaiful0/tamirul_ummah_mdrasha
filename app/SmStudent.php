...
{
    use HasFactory;
    protected $fillable = ['name_in_tamil'];
    ...
    public static function examCode($exam_id, $sb_id)
    {
        try {
            return SmExam::where('exam_type_id', $exam_id)->where('subject_id', $sb_id)->first();
        } catch (\Exception $e) {
            $data = [];
            return $data;
        }
    ...
    public function studentExtraCurricularRecords()
    {
        return $this->hasMany(StudentExtraCurricularRecord::class, 'student_id', 'id')->where('is_promote', 0)->where('active_status', 1)->orderBy('id', 'DESC');
    }
