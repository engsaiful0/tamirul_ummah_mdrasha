...
trait SidebarDataStore
{
    ...
    {
        {
            ...
            // Check if permission sections exist for this user
            ...
            foreach ($permissionSections as $item) {
                $existingSection = Permission::where('route', $item['route'])
            ...
            // If sidebar exists but permission sections are missing, ensure they're created
            if ($exit && $missingSections) {
                foreach ($permissionSections as $item) {
                    storePermissionData($item, $user->id);
                }
                // Recreate sidebar records for permission sections
    ...
    function resetSidebarStore()
    {
        ...
        $student_sections = ["student_info", "fees", "fees_collection", "transport", "dormitory", "library", "homework", "behaviour_records", "quick_payment"];
    ...
    function parentId($sidebar)
    {
        ...
        ->where(function($q) {
            // For permission sections, ensure we get the one with correct user_id
            $q->where('user_id', auth()->user()->id)->orWhereNull('user_id');
        })
        // ->where('menu_status', 1)
        ...
        if (in_array(trim($sidebar->route), $this->paidModuleRoutes())) {
