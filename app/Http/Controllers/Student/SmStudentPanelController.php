...
class SmStudentPanelController extends Controller
{
    ...
    //     // You can also rethrow the exception if you want to let it propagate further
    //     // throw $e;
    // }
            ...
            if ($student) {
                ...
                if (($request->sibling_id == 0 || $request->sibling_id == 1) && $request->parent_id == "") {
                    if ($request->filled('mothers_email')) {
                        ...
                        $user_parent =  $this->addUser($student_detail->parents->user_id, 3, $username, $request->mothers_email, $phone_number);
                        $user_parent->toArray();
                    }
                    ...
                    if ($request->filled('mothers_email')) {
                        $username = $request->mothers_phone ? $request->mothers_phone : $request->mothers_email;
                        $phone_number = $request->mothers_phone;
                        $user_parent = $this->addUser(null, 3, $username, $request->mothers_email, $phone_number);
                        $user_parent->toArray();
                    }
                }
                // end
    ...
    public function studentDashboard(Request $request, $id = null)
    {
        try {
             ...
             $role_id = $user->role_id;
                ...
                if(!userPermission('student-dashboard') && $role_id == '2') {
            ...
            if(moduleStatusCheck('University')){
                ...
                // echo $student_detail->id;exit();
                $records = StudentRecord::where('student_id',$student_detail->id)->get();
            }

            $datas = StudentRecord::where('student_id',$student_detail->id)->first();
            ...
            $assignSubjects = DB::table('sm_assign_subjects')
                ...
                ->where('sm_assign_subjects.class_id', '=', $datas['class_id'])
                ->where('sm_assign_subjects.section_id', '=', $datas['section_id'])
                ->where('sm_assign_subjects.academic_id', getAcademicId())
                ->where('sm_assign_subjects.school_id', Auth::user()->school_id)->get();


            // print_r($assignSubjects);exit();

            // Get Exam Details 
            // DB::enableQueryLog();
            $current_exam = DB::table('sm_exam_settings')
                ->leftjoin('sm_exam_schedules', 'sm_exam_schedules.exam_term_id', '=', 'sm_exam_settings.exam_type')
                ->where('sm_exam_settings.school_id', Auth::user()->school_id)
                ->where('sm_exam_settings.academic_id', getAcademicId())
                ->where('sm_exam_schedules.class_id', $datas['class_id'])
                ->where('sm_exam_schedules.section_id', $datas['section_id'])
                ->where('sm_exam_schedules.academic_id', getAcademicId())
                ->where('sm_exam_schedules.school_id', Auth::user()->school_id) 
                // ->orderBy('sm_exam_settings.publish_date', 'desc')
                ->pluck('sm_exam_settings.exam_type')
                ->last();

            // echo $current_exam;


            $previous_exam = DB::table('sm_exam_settings')
                ->leftJoin('sm_exam_schedules', 'sm_exam_schedules.exam_term_id', '=', 'sm_exam_settings.exam_type')
                ->where('sm_exam_settings.school_id', Auth::user()->school_id)
                ->where('sm_exam_settings.academic_id', getAcademicId())
                ->where('sm_exam_schedules.class_id', $datas['class_id'])
                ...
                ->where('sm_exam_schedules.academic_id', getAcademicId())
                ->where('sm_exam_schedules.school_id', Auth::user()->school_id) 
                // ->orderBy('sm_exam_settings.publish_date', 'desc') // Assuming there's a created_at column
                ->pluck('sm_exam_settings.exam_type')
                ->take(2) 
                ->skip(1) 
                ->first();

            // echo $previous_exam;exit();
            // dd(DB::getQueryLog());
            // echo $current_exam;exit();

            $chart_data = "";
            foreach($assignSubjects as $subject){
                // DB::enableQueryLog();
                $getMarkCurrentExam = SmResultStore::where([
                                ['exam_type_id', $current_exam],   
                                ['student_id', $student_detail->id],
                                ['subject_id', $subject->subject_id],
                            ])->first();
                
                $totalMarksCurrentExam = $getMarkCurrentExam ? $getMarkCurrentExam['total_marks'] : 0;

                $getMarkPrevExam = SmResultStore::where([
                                ['exam_type_id', $previous_exam],   
                                ['student_id', $student_detail->id],
                                ['subject_id', $subject->subject_id],
                            ])->first();
                
                $totalMarksPrevExam = $getMarkPrevExam ? $getMarkPrevExam['total_marks'] : 0;

                $chart_data .= "{ y: '" . $subject->subject_name . "', a: '" . $totalMarksPrevExam . "', b: '" . $totalMarksCurrentExam . "' },";
            }
            // exit();
            // dd(DB::getQueryLog());
            ...
            return view('backEnd.studentPanel.studentProfile', compact('totalSubjects', 'totalNotices', 'online_exams', 'teachers', 'issueBooks', 'homeworkLists', 'attendances', 'driver', 'student_detail', 'fees_assigneds', 'fees_discounts', 'exams', 'documents', 'timelines', 'siblings', 'grades', 'events', 'holidays', 'sm_weekends','records', 'routineDashboard','chart_data'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function studentResult()
    {
        try {
            ...
            $id = SmStudent::where('email', $studentEmail)->value('id');
            ...
            if(moduleStatusCheck('University')){
                ...
                return view('backEnd.studentPanel.student_result', compact('student_detail', 'exams', 'grades', 'exam_terms', 'failgpaname', 'optional_subject_setup', 'student_optional_subject', 'maxgpa', 'records','studentDetails','studentRecordDetails','studentRecords', 'max_gpa', 'fail_gpa_name'));
            }else{
                return view('backEnd.studentPanel.student_result', compact('student_detail', 'exams', 'grades', 'exam_terms', 'failgpaname', 'optional_subject_setup', 'student_optional_subject', 'maxgpa', 'records','max_gpa', 'fail_gpa_name'));
            }
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
