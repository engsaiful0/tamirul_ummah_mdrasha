...
class SmApiController extends Controller
{
     ...
     public function mobileLogin(Request $request)
    {
        ...
        try {
            ...
            // Check if email matches fathers_email, mothers_email, or guardians_email in sm_parents table
            if(!$users->count()) {
                $parent = SmParent::where(function($query) use ($request) {
                    $query->where('fathers_email', $request->email)
                          ->orWhere('mothers_email', $request->email)
                          ->orWhere('guardians_email', $request->email);
                })->first();
                
                if($parent && $parent->user_id){
                    $users = User::where('id', $parent->user_id)->get();
                }
            }
    ...
    public function upiList(Request $request){
        try {
            $upi=SmUpiAccount::where('active_status',1)
                // ->where('academic_id', SmAcademicYear::SINGLE_SCHOOL_API_ACADEMIC_YEAR())
                ->where('school_id',1)->get(['id','upi_id','upi_name']);
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                $data = [];
                $data['upi_accounts'] = $upi->toArray();
                return ApiBaseMethod::sendResponse($data, "UPI Account Details");
            }
        } catch (\Throwable $th) {
    ...
    public function subjectsNameTeacher(Request $request, $id,$class_id,$section_id)
    {
        $teacher = DB::table('sm_staffs')
            ->where('user_id', '=', $id)
            ->first();
    ...
    public function adminDashboardDetails($school_id)
    {
        ...
        try {
                    ...
                    $all_students = SmStudent::where('active_status', 1)
                                ->where('school_id', $school_id)->get();
                    $data['institute_overview'] =[
                        'total_students' =>$all_students->count(),
                        'total_parents' => $all_students->whereNotNull('parent_id')->unique('parent_id')
                            ->count(),

                        'total_teachers' => $all_staffs->where('role_id', 4)->count(),
                        'total_staffs' =>$all_staffs->count()
                    ];

                    $today_income = SmAddIncome::where('date', '=', date('Y-m-d'))
                        ->where('name', '!=', 'Fund Transfer')
                        ->where('active_status', 1)
                        ->where('school_id', $school_id)
                        ->sum('amount');
                    $today_expense = SmAddExpense::where('date', '=', date('Y-m-d'))
                        ->where('name', '!=', 'Fund Transfer')
                        ->where('active_status', 1)
                        ->where('school_id', $school_id)
                        ->sum('amount');
                    ...
                    $today_income_details = SmStudent::Select('sm_students.id','sm_students.full_name','sm_students.student_photo')
                        ->leftjoin('sm_fees_payments','sm_fees_payments.student_id','=','sm_students.id')
                        ->leftjoin('sm_add_incomes','sm_add_incomes.fees_collection_id','=','sm_fees_payments.id')
                        ->where('sm_add_incomes.name', '!=', 'Fund Transfer')
                        ->where('sm_add_incomes.active_status', 1)
                        ->where('sm_add_incomes.school_id', $school_id)
                        ->where('sm_add_incomes.date', '=', date('Y-m-d'))
                        ->groupBy('sm_students.id')->get();   
                    $data['today_income_details'] = $today_income_details;   

                    $today_expense_details = SmAddExpense::where('date', '=', date('Y-m-d'))
                        ->where('name', '!=', 'Fund Transfer')
                        ->where('active_status', 1)
                        ->where('school_id', $school_id)
                        ->select('name')->groupBy('name')->get();  
                    $data['today_expense_details'] = $today_expense_details;   

                    // Student Leave
                    $today_std_leave_details = SmStudentAttendance::Select('sm_students.id','sm_students.full_name','sm_students.student_photo','student_records.class_id','student_records.section_id','sm_sections.section_name','sm_classes.class_name')
                        ->leftjoin('sm_students','sm_students.id','=','sm_student_attendances.student_id')
                        ->leftjoin('student_records','student_records.id','=','sm_student_attendances.student_record_id')
                        ->leftjoin('sm_sections','sm_sections.id','=','student_records.section_id')
                        ->leftjoin('sm_classes','sm_classes.id','=','student_records.class_id')
                        ->where('sm_student_attendances.attendance_date', '=', date('Y-m-d'))
                        ->where('sm_student_attendances.attendance_type','A')
                        ->where('sm_students.school_id', $school_id)
                        ->where('sm_students.active_status', 1)
                        ->where('sm_students.academic_id', getAcademicId())
                        ->orderBy('sm_student_attendances.created_at','DESC')->limit(5)->get();
                    $data['student_leave_details'] = $today_std_leave_details;

                    $data['student_leave_count']=SmStudentAttendance::where('attendance_date', '=', date('Y-m-d'))
                                                ->where('attendance_type','A')
                                                ->where('school_id', $school_id)
                                                ->where('active_status', 1)
                                                ->where('academic_id', getAcademicId())->count();


                    $today_staff_leave_details = SmStaffAttendence::Select('sm_staffs.id','sm_staffs.full_name','sm_staffs.staff_photo','roles.name as role_name')
                            ->leftjoin('sm_staffs','sm_staffs.id','=','sm_staff_attendences.staff_id')
                            ->leftjoin('roles','roles.id','=','sm_staffs.role_id')
                            ->where('sm_staff_attendences.attendence_date', '=', date('Y-m-d'))
                            ->where('sm_staff_attendences.attendence_type','A')
                            ->where('sm_staffs.active_status', 1)
                            ->where('sm_staffs.school_id', $school_id)
                            ->orderBy('sm_staff_attendences.created_at','DESC')
                            ->limit(3)->get();
                    $data['staff_leave_details'] = $today_staff_leave_details;

                    $data['staff_leave_count']=SmStaffAttendence::where('attendence_date', '=', date('Y-m-d'))
                                                ->where('attendence_type','A')
                                                ->where('school_id', $school_id)
                                                ->count();
                    $data['total_leave_count']=$data['student_leave_count']+$data['staff_leave_count'];

                    $all_homeworks = SmHomework::query();
                    $all_homeworks->with('classes','sections','subjects','users');
                    $all_homeworks->where('school_id',$school_id)->orderby('id','DESC')->where('academic_id', getAcademicId())
                    ->where('submission_date', '=', date('Y-m-d'));

                    // $data['Homework_list']=$homeworkLists = $all_homeworks->get();
                    $homeworkLists = $all_homeworks->get();

                    $hw_submit=[];
                    $hw_list=[];$k=0;
                    foreach($homeworkLists as $hw){
                        $hw_students = SmStudent::select('sm_students.*')
                        ->leftjoin('student_records','student_records.student_id','=','sm_students.id')
                        ->where('student_records.class_id',$hw->class_id)
                        ->where('student_records.section_id',$hw->section_id)
                        ->where('sm_students.active_status', 1)
                        ->where('sm_students.school_id', $school_id)->limit(4)->get();
                        $hw_submit['hw_submit_students']=$hw_students;
                        $hw_list[$k]['class_name']=$hw['classes']->class_name;
                        $hw_list[$k]['section_name']=$hw['sections']->section_name;
                        $hw_list[$k]['subject']=$hw['subjects']->subject_name;
                        $hw_list[$k]['marks']=$hw->marks;
                        $k++;
                    }

                    $data['homework_list']=$hw_list;

                    // $data['Homework_submitted_students']=$hw_submit;

                    $class_per_students = studentRecord::select(DB::raw('COUNT(*) as count'), 'class_id')
                    ->where('school_id', $school_id)
                    ->where('active_status', 1)
                    ->where('academic_id', getAcademicId())
                    ->groupBy('class_id')->get();
                    
                    $class_students = json_decode($class_per_students);
                    $class_attandance=[];
                    $i=0;
                    foreach($class_students as $student_count)
                    {
                        $student_present = SmStudentAttendance::where('attendance_date', '=', date('Y-m-d'))
                        ->where('attendance_type','!=','A')
                        ->where('class_id',$student_count->class_id)
                        ->where('school_id', $school_id)
                        ->where('active_status', 1)
                        ->where('academic_id', getAcademicId())->count();
                         $class_attandance[$i]['total_student']=$student_count->count;
                         $class_attandance[$i]['total_present']=$student_present;
                         $class_attandance[$i]['class']=$student_count->class_id;
                         $class = StudentRecord::getStudentclass1($class_attandance[$i]['class']);
                         $class_attandance[$i]['class_name']=$class->class_name;
                         $i++;
                    }
                    $data['class_attandance']=$class_attandance;
                    ...
                    {
                        $date = Carbon::parse($notice->publich_on);
                        $publich_on = $date->format('d M Y');
                        $notices[$j]['announcement_date']=$publich_on;
                        $notices[$j]['notice_title']=$notice->notice_title;
                        $notices[$j]['notice_message']=$notice->notice_message;
                        $j++;
                    }

                    $data['announcement']=$notices;
                    
                    return ApiBaseMethod::sendResponse($data, 'Admin Dashboard Details');
    ...
    public function getFullDomainUrl(Request $request){
        // print_r($request->domain_name);exit();
        $data=array();
        if(isset($request->domain_name) || !empty($request->domain_name)){
            $school = SmSchool::where('domain', $request->domain_name)->first();
            if ($school) {
                $data['school_name']=$school->school_name;
                $data['domain_name']=$school->domain;

                // $protocol = app()->environment('local') ? 'http://' : 'https://';
                $protocol='https://';
                $data['redirect_url']=$protocol.$school->domain.'.'.$rootDomain = config('app.domain');
                // $data['domain_details']=$school_details;
                return ApiBaseMethod::sendResponse($data, 'Domain Details');
            } else {
                return ApiBaseMethod::sendError('Domain not found');
            }
        }else{
            return ApiBaseMethod::sendError('Please enter domain name');
        }
    }
}
