    ...
    public function childInfo(Request $request, $user_id)
    {
        ...
        try {
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                ...
                if(empty($user)){
                     return ApiBaseMethod::sendError('Error.', "User Not found");
                }
                ...
                $siblings = SmStudent::select('sm_students.*','student_sibling_records.relationship')
                ->join('student_sibling_records','student_sibling_records.sibling_id','=','sm_students.id')
                ->where('student_sibling_records.student_id',$user->id)->get();

                // $data['siblings'] = $siblings;
                $siblings_data=array();
                foreach ($siblings as $sibling) {
                    $sib['id']= $sibling->id;
                    $sib['admission_no']= $sibling->admission_no;
                    $sib['roll_no']= $sibling->roll_no;
                    $sib['first_name'] = $sibling->first_name;
                    $sib['last_name']= $sibling->last_name;
                    $sib['full_name']= $sibling->full_name;
                    // $sib['date_of_birth']= $sibling->date_of_birth;
                    // $sib['email']= $sibling->email;
                    // $sib['mobile']= $sibling->mobile;
                    // $sib['admission_date']= $sibling->admission_date;
                    $sib['student_photo']= $sibling->student_photo;
                    // $sib['age']= $sibling->age;
                    $sib['active_status']= $sibling->active_status;
                    // $sib['custom_field']= $sibling->custom_field;
                    // $sib['custom_field_form_name']= $sibling->custom_field_form_name;
                    // $sib['created_at']= $sibling->created_at;
                    // $sib['updated_at']= $sibling->updated_at;
                    // $sib['parent_id']= $sibling->parent_id;
                    // $sib['user_id']= $sibling->user_id;
                    // $sib['role_id']= $sibling->role_id;
                    // $sib['gender_id']= $sibling->gender_id;
                    // $sib['school_id']= $sibling->school_id;
                    // $sib['academic_id']= $sibling->academic_id;
                    $sib['class_id'] = $sibling->studentRecord ? $sibling->studentRecord->class->class_name : null;
                    $sib['section_id'] = $sibling->studentRecord ? $sibling->studentRecord->section->section_name : null;
                    $sib['gender']= $sibling->gender->base_setup_name;
                    $sib['relationship']= $sibling->relationship;
                    $siblings_data []=$sib;
                }

                $data['siblings'] = $siblings_data;
