...
class DatatableQueryController extends Controller
{
        ...
        try {
            if (Auth::user()->role_id == 1) {
                ...
                $staffs->withOutGlobalScope(ActiveStatusSchoolScope::class)->where('school_id', Auth::user()->school_id)
                    ...
                    $staffs =  $staffs->orderByDesc('id')->get();;
            } else {
            ...
            return Datatables::of($staffs)
                ...
                ->addColumn('action', function ($row) {
                    $btn = '<div class="dropdown CRM_dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                            <a class="dropdown-item" target="_blank" data-toggle="modal" onclick="downloadPayslipModal('.$row->id.')">' . app('translator')->get('hr.download_payslip') . '</a>
                                            <a class="dropdown-item" target="_blank" href="' . route('view_payslip', [$row->id]) . '">' . app('translator')->get('hr.view_payslip') . '</a>
    ...
    public function getPayrollStaffList(Request $request)
    {  
        try {
            if (Auth::user()->role_id == 1) {
                $staffs = SmStaff::query();
            ...
            return Datatables::of($staffs)
                ...
                ->addColumn('action', function ($row) {
                    $btn = '<div class="dropdown CRM_dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                            <a class="dropdown-item" target="_blank" data-toggle="modal" onclick="earningModal('.$row->id.')">Earnings</a>
                                            <a class="dropdown-item" target="_blank" data-toggle="modal" onclick="deductionModal('.$row->id.')">Deductions</a>
                                            <a class="dropdown-item" target="_blank" href="' . route('viewStaff', [$row->id]) . '">' . app('translator')->get('common.view') . '</a>' .
                        (userPermission('editStaff') === true ? '<a class="dropdown-item" href="' . route('editStaff', [$row->id]) . '">' . app('translator')->get('common.edit') . '</a>' : '') .
        ...
        if($request->ajax()){
            ...
            ->addColumn('invoice', function($row){
                if(moduleStatusCheck('University')){
                  ...
                  return  $row->fees_type_id.'/'.$row->id;
                }
            ...
            ->addColumn('action', function ($row) {
                ...
                if($row->payment_mode=='bank' || $row->payment_mode=='cheque'){
                    $bank_btn='<a onclick="rejectExtraFeesPayment(' . $row->id . ','.$row->recordDetail->id.','.$row->amount.','.$row->student_id.');" class="dropdown-item" href="#" data-toggle="modal" data-id="' . $row->id . '"  >' . app('translator')->get('common.reject') . '</a>';
                }else{
                    $bank_btn='';
                }
                ...
                $btn = '<div class="dropdown CRM_dropdown">
                    ...
                    (! moduleStatusCheck('University') && directFees() == false  ? (Config::get('app.app_sync') ? '<span  data-toggle="tooltip" title="Disabled For Demo "><a  class="dropdown-item" href="#"  >' . app('translator')->get('common.disable') . '</a></span>' :
                        '<a onclick="deleteFeesPayment(' . $row->id . ');"  class="dropdown-item" href="#" data-toggle="modal" data-id="' . $row->id . '"  >' . app('translator')->get('common.delete') . '</a>') : '') .$bank_btn.
    ...
    public function ajaxIncomeList(Request $request){
        ...
        if($request->ajax()){
            ...
            ->addColumn('action', function ($row) {
                ...
                if($row->name!='Fees Collect' && $row->name!='Extra Fees Collect'){
                    $div_edit=(userPermission('add_income_edit') === true ? '<a class="dropdown-item" href="' . route('add_income_edit', [$row->id]) . '">' . __('common.edit') . '</a>' :'');
                }else{
                    $div_edit='';
                }
                $btn = '<div class="dropdown CRM_dropdown"><button type="button" class="btn dropdown-toggle" data-toggle="dropdown">' . app('translator')->get('common.select') . '</button><div class="dropdown-menu dropdown-menu-right">'.$div_edit
    ...
    public function ajaxExpenseList(Request $request){
        ...
        public function ajaxPendingLeave(Request $request){
             if($request->ajax()){
                if (checkAdmin()) {
                    $apply_leaves = SmLeaveRequest::with('leaveType','leaveDefine','staffs','student','user:id,full_name')->where([['active_status', 1], ['approve_status', '!=', 'A']])
                                    ->where('school_id', Auth::user()->school_id)
                                    ->where('academic_id',getAcademicId())
                                    ->get();
                    ...
                    if($class_teacher){
                        $leaves = SmLeaveRequest::where([
                            ...
                            ->with('leaveType','leaveDefine','staffs','student','user:id,full_name')
                            ->where('school_id', Auth::user()->school_id)
                            ->where('academic_id',getAcademicId())
                            ->first();
                            ...
                            if($leaves){
                                $apply_leaves = SmLeaveRequest::with('leaveType','leaveDefine','staffs','student','user:id,full_name')->with(array('student' => function($query)use($smAssignClassTeacher) {
                        ...
                        $apply_leaves = SmLeaveRequest::with('leaveDefine','leaveDefine','staffs','student','user:id,full_name')->where([
                            ['active_status', 1], 
                            ['approve_status', '!=', 'A'],
                    ...
                    $apply_leaves = SmLeaveRequest::with('leaveDefine','leaveType','staffs','student','leaveType','user:id,full_name')->where([['active_status', 1], ['approve_status', '!=', 'A']])
                    ->where('school_id', Auth::user()->school_id)
                    ->where('academic_id',getAcademicId())
                    ->get();
                }else{
                ...
                if (!isset($apply_leaves) || $apply_leaves=='' || $apply_leaves == null || $apply_leaves->isEmpty()) {
                    return datatables([])->make(true); // Return empty dataset if no records
                }
        ...
        public function ajaxApproveLeave(Request $request){
              if($request->ajax()){
                if (Auth::user()->role_id == 1) {
                    $apply_leaves = SmLeaveRequest::with('leaveType','leaveDefine','staffs','student','user:id,full_name')->where([['active_status', 1], ['approve_status', '!=', 'P']])->where('school_id', Auth::user()->school_id)->where('academic_id', getAcademicId())->get();
                } elseif(auth()->user()->staff) {
                    $apply_leaves = SmLeaveRequest::with('leaveType','leaveDefine','staffs','student','user:id,full_name')->where([['active_status', 1], ['approve_status', '!=', 'P'], ['staff_id', '=', auth()->user()->id]])->where('academic_id', getAcademicId())->get();
                }
        ...
        //             $apply_leaves = SmLeaveRequest::with('leaveType','leaveDefine','staffs','student')->where([['active_status', 1], ['approve_status', '!=', 'P'], ['staff_id', '=', auth()->user()->id]])->where('academic_id', getAcademicId())->get();
        //         }
            ...
            if($request->ajax()){
                ...
                if ($request->book_number) {
                    $all_issue_books->whereHas('books', function ($query) use ($request) {
                        $query->where('book_number', $request->book_number);
                    });
                }

                if ($request->subject_id) {
                    $all_issue_books->whereHas('books', function ($query) use ($request) {
                        $query->where('book_subject_id', $request->subject_id);
                    });
                }
                
                $all_issue_books->join('sm_books', 'sm_book_issues.book_id', '=', 'sm_books.id')
                ->leftJoin('sm_library_members', 'sm_book_issues.member_id', '=', 'sm_library_members.student_staff_id')
                ->leftJoin('library_subjects', 'library_subjects.id', '=', 'sm_books.book_subject_id')
                ->leftJoin('users', 'users.id', '=', 'sm_library_members.student_staff_id')
                ->where('sm_book_issues.school_id', Auth::user()->school_id)
                ->where('sm_book_issues.issue_status', 'I')
                ->select('sm_book_issues.*', 'sm_books.book_title', 'sm_books.book_number', 'sm_books.isbn_no', 'sm_books.author_name', 'library_subjects.subject_name', 'sm_library_members.member_type', 'sm_library_members.student_staff_id');
                ...
                ->addColumn('full_name', function($row){
                    ...
                    if ($row->member_type == 2) {
                        $student = \App\SmStudent::where('user_id', $row->student_staff_id)->first();
                        $full_name = $student ? $student->full_name : '';
                    } elseif($row->member_type == 3){
                        $parent = \App\SmParent::where('user_id', $row->student_staff_id)->first();
                        $full_name = $parent ? $parent->guardians_name : '';
                    } else {
                        $staff = \App\SmStaff::where('user_id', $row->student_staff_id)->first();
        ...
        public function studentTransportReportAjax(Request $request){
            if($request->ajax()){
                ...
                if($request->has('route') && $request->route != ""){
                    ...
                    $students->whereNotNull('route_list_id')->where('route_list_id', '!=', '');
                }
                ...
                if($request->has('vehicle') && $request->vehicle != ""){
                    ...
                    $students->whereNotNull('vechile_id')->where('vechile_id', '!=', '');
                }
                
                // Filter by class and section using StudentRecord
                if($request->has('class') && $request->class != ""){
                    ...
                    if($request->has('section') && $request->section != ""){
                        $student_ids->where('section_id', $request->section);
                    }
                    
                    $student_ids = $student_ids->pluck('student_id')->unique()->toArray();
                    $students->whereIn('id', $student_ids);
                }
                
                // University module support
                if(moduleStatusCheck('University')){
                    ...
                    $university_student_ids = $student_records->distinct('student_id')->pluck('student_id')->toArray();
                    if(!empty($university_student_ids)){
                        $students->whereIn('id', $university_student_ids);
                    }
                }
                
                // Eager load relationships
                $students->with(['studentRecord' => function($q) {
                    if(!moduleStatusCheck('University')){
                        $q->where('academic_id', getAcademicId());
                    }
                    $q->where('is_promote', 0)
                      ->with('class', 'section');
                }, 'parents', 'route', 'vehicle', 'drivers']);
                
                // Only get students with vehicle assigned
                $students->whereHas('vehicle');
                
                return Datatables::of($students)
                ->addIndexColumn()
                ->addColumn('class_section', function ($row) {
                    $studentRecord = $row->studentRecord;
                    if($studentRecord && $studentRecord->class && $studentRecord->section){
                        return $studentRecord->class->class_name .' ('.$studentRecord->section->section_name.')';
                    }
                    return '-';
                })
                ->addColumn('mobile', function ($row) {
        ...
        public function ajaxExtraFeesPayment(Request $request){

            if($request->ajax()){
                $date_from = date('Y-m-d', strtotime($request->date_from));
                $date_to = date('Y-m-d', strtotime($request->date_to));
                $fees_payments = SmExtraCurricularFeesPayment::query();
                // $fees_payments = $fees_payments->when(directFees(), function($q){
                //     $q->whereNotNull('installment_payment_id');
                // });
                ...
                if($request->extra_class){
                    $fees_payments->whereHas('recordDetail', function ($q) use($request){
                        return $q->where('extra_class_id', $request->extra_class);
                        });
                }

                if($request->pay_mode){
                    $fees_payments->whereHas('recordDetail', function ($q) use($request){
                        return $q->where('payment_mode', $request->pay_mode);
                        });
                }
                // if($request->section){
                //     $fees_payments->whereHas('recordDetail', function ($q) use($request){
                ...
                $fees_payments = $fees_payments->whereHas('recordDetail')->with('recordDetail.studentDetail','recordDetail.classes_student','recordDetail.section','studentInfo','recordDetail.extraClass')->get();
                ...
                ->addColumn('class_sec', function ($row) {
                        return $row->recordDetail->classes_student->class_name;
                })
                ->addColumn('extracurricular_class', function ($row) {
                        return $row->recordDetail->extraClass->class_name;
                })
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn = '<div class="dropdown CRM_dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right">
                                                <a data-modal-size="modal-lg" target="_blank" class="dropdown-item" href="' . route('extraclass_fees_collect_student_wise', [$row->recordDetail->id]) . '">' . app('translator')->get('common.view') . '</a> '.$bank_btn.'
        ...
        public function ajaxExtraFeesDue(Request $request){

            if($request->ajax()){
                // DB::enableQueryLog();
                $fees_payments = StudentExtraCurricularRecord::query();
                if($request->class){
                        $fees_payments->where('student_extra_curricular_records.class_id', $request->class);
                }

                if($request->extra_class){
                        $fees_payments->where('student_extra_curricular_records.extra_class_id', $request->extra_class);
                }

                if ($request->keyword != "") {
                    $fees_payments->whereHas('studentDetail', function($q) use($request) {
                        $q->where('full_name', 'like', '%' . $request->keyword . '%')->orWhere('admission_no', $request->keyword)->orWhere('roll_no', $request->keyword);
                    });                    
                }
                
                $fees_payments->leftJoin('sm_extraclass_fees', function($join) {
                  $join->on('student_extra_curricular_records.id', '=', 'sm_extraclass_fees.extra_curricular_record_id');
                  // $join->where('sm_extraclass_fees.id', '>', 0); // Add this condition
                });
                $fees_payments->where('sm_extraclass_fees.id', '>', 0);
                $fees_payments->where('student_extra_curricular_records.status', '!=', 1);
                $fees_payments = $fees_payments
                                ->with('studentDetail','classes_student','section','extraClass')
                                ->get();
                // dd(DB::getQueryLog()); 
                ...
                ->addIndexColumn()
                ->addColumn('invoice', function($row){
                      return  $row->id.'<input type="hidden" name="student_list[]" value="'.$row->studentDetail->id.'"> <input type="hidden" name="due_amount[]" value="'.($row->fees_amount-$row->paid_amount).'">';
                })
                ->addColumn('fees_amount', function ($row) {
                    return generalSetting()->currency_symbol. ' '.$row->fees_amount-$row->paid_amount;
                })
                ->addColumn('class_sec', function ($row) {
                        return $row->classes_student->class_name;
                })
                ->addColumn('extracurricular_class', function ($row) {
                        return $row->extraClass->class_name;
                })
                ->addColumn('action', function ($row) {
                    $btn = '<div class="dropdown CRM_dropdown">
                                        <button type="button" class="btn btn-secondary dropdown-toggle" data-toggle="dropdown">' . app('translator')->get('common.select') . '</button>
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right">
                                                <a data-modal-size="modal-lg" target="_blank" class="dropdown-item" href="' . route('extraclass_fees_collect_student_wise', [$row->id]) . '">' . app('translator')->get('common.view') . '</a></div>
                                    </div>';
