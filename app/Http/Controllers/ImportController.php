        ...
        try {
            ...
            if (!empty($bulk_staffs)) {
                ...
                foreach ($bulk_staffs as $key => $singleStaff) {
                    ...
                    if ($role_id=='') {
                            Toastr::error('Invalid Role!', 'Failed');
                                return redirect()->back();
                            //continue;
                        }

                    $staffno_check = SmStaff::where('staff_no', (int) $singleStaff->staff_no)->where('school_id', Auth::user()->school_id)->get();
                    if ($staffno_check->count() > 0) {
                        Toastr::error('Staff number should be unique!', 'Failed');
                        return redirect()->back();
                    }
                    ...
                    {
                       $staffemail_check = SmStaff::where('email', $singleStaff->email)->where('school_id', Auth::user()->school_id)->get();
                       if ($staffemail_check->count() > 0) {
                            Toastr::error('Email should be unique.', 'Failed');
                            return redirect()->back();
                        }
                    }
