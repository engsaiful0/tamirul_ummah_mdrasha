...
class LoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if ($school->id !=1 && $school->active_status !=1) {           
            ...
            return redirect()->route('login')->with('error','Your Institution is not Active, Please contact with administrator.', 'Failed');
        }
        if (config('app.app_sync') && $request->auto_login){
            ...
            if(!$user){
                $parent = SmParent::where(function($query) use ($request, $school) {
                ...
                if($parent && $parent->user_id){
                    $user = User::where('id', $parent->user_id)->where('school_id', $school->id)->first();
                }
            }
        ...
        if ($logged_in) {
            ...
            if (!$school->active_status) {
                ...
                // return redirect()->route('login');
                return redirect()->route('login')->with('error','Your Institution is not Approved, Please contact with administrator.', 'Failed');
            }
            ...
            if (!Auth::user()->access_status || !Auth::user()->active_status) {
                ...
                // return redirect()->route('login');
                return redirect()->route('login')->with('error','You are not allowed, Please contact with administrator.', 'Failed');
            }
    ...
    public function migrate_command(Request $request)
    {  
        try {
            DB::purge('second_db');
            ...
            $res = DB::setDefaultConnection($dynamicConnectionName);

            Artisan::call('cache:clear'); 
            Artisan::call('migrate', array('--database'=>'second_db'));
