...
use App\Models\StudentSiblingRecord;
...
class SmStudentAdmissionController extends Controller
{
    ...
    {
        // If request is AJAX, return validation errors
        if ($request->ajax()) {
        ...
        $parentInfo = ($request->fathers_name || $request->fathers_phone || $request->fathers_email || $request->mothers_name || $request->mothers_phone || $request->mothers_email || $request->guardians_email || $request->guardians_phone )  ? true : false;
        // add student record
        if ($request->filled('phone_number') || $request->filled('email_address')) {
            ...
            //     ->first();
           //Student Mobile unique check
            if ($request->phone_number != "") {
                $chk = DB::table('sm_students')->where('mobile', $request->phone_number)->where('school_id', Auth::user()->school_id)->count();
                if ($chk >= 1) {
                    Toastr::error('Student mobile should be unique.', 'Failed');
                    return redirect()->back()->withInput();
                }
            }
            $user = User::where('school_id', auth()->user()->school_id)
                ->when($request->filled('email_address'), function ($q) use ($request) {
                    $q->where(function ($q) use ($request) {
                        return $q->where('email', $request->email_address)->orWhere('username', $request->email_address);
                    });
                })               
            ...
            if ($user) {
                if ($user->role_id == 2) {
                    ...
                    if (!$studentRecord) {
                        ...
                        if($request->extraclass)
                        {
                            $this->insertStudentExtraCurricularRecord($request->merge([
                                'student_id' => $user->student->id,
                            ]));
                        }
                        ...
                        {
                            $this->insertStudentSiblingRecord($request->merge([
                                'student_id' => $user->student->id,
                            ]));
                        }
        ...
        try {
            ...
            if ($request->parent_id == "") {
                    $userIdParent = null;
                    $hasParent = null;
                if ($request->filled('mothers_email')) {
                    
                    if (!$staff) {  
                        $user_parent = new User();
                        $user_parent->role_id = 3;
                        $mothers_phone = $request->mothers_phone;
                        $mothers_email = $request->mothers_email;
                        $user_parent->username = $mothers_phone ? $mothers_phone : $mothers_email;
                        $user_parent->full_name = $request->mothers_name ? $request->mothers_name : $request->fathers_name;
                        $user_parent->email = $mothers_email;
                        $user_parent->phone_number = $mothers_phone;
            ...
            // Save dormitory and room only if provided
            if ($request->filled('dormitory_name') && $request->dormitory_name != "") {
                $student->dormitory_id = $request->dormitory_name;
            } else {
                $student->dormitory_id = null;
            }
            
            if ($request->filled('room_number') && $request->room_number != "") {
            ...
            $student->bmi_date = date('Y-m-d', strtotime($request->bmi_date));
            $student->bmi_height = $request->bmi_height;
            $student->bmi_weight = $request->bmi_weight;

            $student->vision_date = date('Y-m-d', strtotime($request->vision_date));
            $student->vision_left = $request->vision_left;
            $student->vision_right = $request->vision_right;

            $student->medical_date = date('Y-m-d', strtotime($request->medical_date));
            $student->medical_name = $request->medical_name;
            $student->medical_comment = $request->medical_comment;

            $student->clinical_date = date('Y-m-d', strtotime($request->clinical_date));
            $student->clinical_name = $request->clinical_name;
            $student->clinical_comment = $request->clinical_comment;

            $student->chest_date = date('Y-m-d', strtotime($request->chest_date));
            $student->chest_size = $request->chest_size;

            $student->dental_date = date('Y-m-d', strtotime($request->dental_date));
            $student->dental_hygiene = $request->dental_hygiene;

            $student->allergies_date = date('Y-m-d', strtotime($request->allergies_date));
            $student->allergies_name = $request->allergies_name;
            $student->allergies_comment = $request->allergies_comment;

            $student->health_issue_date = date('Y-m-d', strtotime($request->health_issue_date));
            //$student->health_issue_name = $request->health_issue_name;
            $student->health_issue_type = $request->health_issue_type;
            $student->health_issue_comment = $request->health_issue_comment;
            $student->health_issue_doctor = $request->health_issue_doctor;

            $student->immunization_date = date('Y-m-d', strtotime($request->health_immunization_date));
            $student->immunization_name = $request->health_immunization_name;
            $student->immunization_type = $request->health_immunization_type;
            $student->immunization_comment = $request->health_immunization_comment;

            $student->mother_tounge = $request->mother_tongue;
    ...
    public function edit(Request $request, $id)
    {
        try {
            ...
            $data['siblings'] = SmStudent::select('sm_students.*')
            ->join('student_sibling_records','student_sibling_records.sibling_id','=','sm_students.id')
            ->where('student_sibling_records.student_id',$id)->get();
            ...
            $student_extra_classes = StudentExtraCurricularRecord::where('student_id',$id)->select('extra_class_id')->get(); 
            ...
            $data['extra_class_ids'] = json_decode(json_encode($extra_class_array), true);
    ...
    public function updateStudentInfo($request, $studentRecord = null)
    {
        ...
        try {
            ...
            if (($request->sibling_id == 0 || $request->sibling_id == 1) && $request->parent_id == "") {
                if ($request->filled('mothers_email')) {
                    $username = $request->mothers_phone ? $request->mothers_phone : $request->mothers_email;
                    $phone_number = $request->mothers_phone;
                    $user_parent = $this->add_user($parentUserId, 3, $username, $request->mothers_email, $phone_number, $request->mothers_name ? $request->mothers_name : $request->fathers_name);
                }
                ...
                if ($request->filled('mothers_email')) {
                    $username = $request->mothers_phone ? $request->mothers_phone : $request->mothers_email;
                    $phone_number = $request->mothers_phone;
                    $user_parent = $this->add_user(null, 3, $username, $request->mothers_email, $phone_number, $request->mothers_name ? $request->mothers_name : $request->fathers_name);
                }
            }
            ...
            if ($request->sibling_id == 0 && $request->parent_id != "") {
                ...
                // Update existing parent when parent_id is provided
                if ($parentInfo && $request->parent_id) {
                    $parent = SmParent::find($request->parent_id);
                    if ($parent) {
                        if ($user_parent) {
                ...
                if ($parentInfo) {
                    ...
                    // Also handle case where parent exists but wasn't caught by above conditions
                    if (!isset($parent) && $student_detail->parent_id) {
                        $parent = SmParent::find($student_detail->parent_id);
                    }
                    
                    if (isset($parent)) {
                        $parent->user_id = $user_parent->id ?? $parent->user_id ?? null;
                        $parent->fathers_name = $request->fathers_name;
                        $parent->fathers_mobile = $request->fathers_phone;
                        $parent->fathers_occupation = $request->fathers_occupation;
                        $parent->fathers_email = $request->fathers_email;
            ...
            // Update class, section, and academic year if provided
            ...
            // Update or create StudentRecord for the current academic year
            if ($request->filled('session') && $request->filled('class') && $request->filled('section')) {
                $academic_id = $request->session;
                $class_id = $request->class;
                $section_id = $request->section;
                
                // Find existing record for this academic year or create new one
                $currentRecord = StudentRecord::where('student_id', $student->id)
                ...
                if ($currentRecord) {
                    ...
                    $currentRecord->class_id = $class_id;
                    $currentRecord->section_id = $section_id;
                    $currentRecord->academic_id = $academic_id;
                    $currentRecord->session_id = $academic_id;
                    if ($request->roll_number && generalSetting()->multiple_roll == 0) {
                        $currentRecord->roll_no = $request->roll_number;
                    }
                    // Set as default if it's the current academic year
                    if ($academic_id == getAcademicId()) {
                        $currentRecord->is_default = 1;
                    }
                    $currentRecord->save();
                    
                    // Unset other default records if this is the current academic year
                    if ($academic_id == getAcademicId()) {
                        StudentRecord::where('student_id', $student->id)
                    ...
                    $newRecord->class_id = $class_id;
                    $newRecord->section_id = $section_id;
                    $newRecord->academic_id = $academic_id;
                    $newRecord->session_id = $academic_id;
                    $newRecord->school_id = auth()->user()->school_id;
                    $newRecord->is_promote = 0;
                    $newRecord->active_status = 1;
                    if ($request->roll_number && generalSetting()->multiple_roll == 0) {
                        $newRecord->roll_no = $request->roll_number;
                    }
                    // Set as default if it's the current academic year
                    if ($academic_id == getAcademicId()) {
                        $newRecord->is_default = 1;
                    }
                    $newRecord->save();
                    
                    // Unset other default records after saving
                    if ($academic_id == getAcademicId()) {
                        StudentRecord::where('student_id', $student->id)
            ...
            } elseif ($studentRecord && generalSetting()->multiple_roll == 0 && $request->roll_number) {
                // Fallback: just update roll number if record exists
    ...
    public function update(SmStudentAdmissionRequest $request)
    {
        ...
        try {
            $validated = $request->validate([
                ...
                'roll_number.integer'  => 'The roll number must be a number.',
                'roll_number.unique'   => 'This roll number has already been taken.',
            ]);
            $studentRecord = StudentRecord::where('student_id', $request->id)->orderBy('created_at')->where('school_id', auth()->user()->school_id)->first();
            ...
            $result = $this->updateStudentInfo($request, $studentRecord);
            // If updateStudentInfo returns a redirect (error case), return it
            if ($result instanceof \Illuminate\Http\RedirectResponse) {
                return $result;
            }
            
            // Clear permission singleton cache to ensure fresh permissions are loaded after redirect
            // This fixes the "Permission Denied" issue when redirecting to student-list
            app()->forgetInstance('permission');
    ...
    private function add_user($user_id, $role_id, $username, $email, $phone_number, $full_name = null)
    {
        try {
            ...
            // Set default password and school_id when creating new user or if password is empty
            if ($user_id == null) {
                $user->password = Hash::make(123456);
                $user->school_id = Auth::user()->school_id;
            } elseif (empty($user->password)) {
                // Set default password for existing users with empty password
                $user->password = Hash::make(123456);
            }
            $user->save();
            return $user;
    ...
    public function view(Request $request, $id, $type = null)
    {
        try {
            ...
            $siblings = SmStudent::select('sm_students.*')
            ->join('student_sibling_records','student_sibling_records.sibling_id','=','sm_students.id')
            ->where('student_sibling_records.student_id',$student_detail->id)->get();
            ...
            $student_extra_classes = DB::table('sm_extra_curricular_classes')
                ->select('sm_extra_curricular_classes.class_name')
                ->join('student_extra_curricular_records','student_extra_curricular_records.extra_class_id','=', 'sm_extra_curricular_classes.id')
                                ->where('sm_extra_curricular_classes.academic_id', '=', old('session', getAcademicId()))
                                ->where('student_extra_curricular_records.student_id',$student_detail->id)
                                ->get();

            $std_extra_curricular_classes = '';
            if($student_extra_classes){
                foreach($student_extra_classes as $std_extra_classes){
                    if($std_extra_curricular_classes==''){
                        $std_extra_curricular_classes = $std_extra_classes->class_name;
                    }else{
                        $std_extra_curricular_classes = $std_extra_curricular_classes.','.$std_extra_classes->class_name;
                    }                    
                }
            }
            ...
            if(moduleStatusCheck('University')){
                ...
                return view('backEnd.studentInformation.student_view', compact('timelines','student_detail', 'driver_info', 'exams', 'siblings', 'grades', 'academic_year', 'exam_terms', 'max_gpa', 'fail_gpa_name', 'custom_field_values', 'sessions', 'records', 'next_labels', 'type','studentRecordDetails','studentDetails','studentRecords','result_setting','assinged_exam_types','std_extra_curricular_classes'));
            }else{
                return view('backEnd.studentInformation.student_view', compact('timelines','student_detail', 'driver_info', 'exams', 'siblings', 'grades', 'academic_year', 'exam_terms', 'max_gpa', 'fail_gpa_name', 'custom_field_values', 'sessions', 'records', 'next_labels', 'type','result_setting', 'attendance', 'subjectAttendance', 'days', 'year', 'month','std_extra_curricular_classes'));
            }
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
    ...
    //Siblings
    public function insertStudentSiblingRecord($request, $pre_record = null)
    {
        StudentSiblingRecord::where('student_id',null)
       ->update([
           'student_id' => $request->student_id
        ]);
    }

    public function insertStudentExtraCurricularRecord($request, $pre_record = null)
    {
        $extraclasses=$request->extraclass;
        foreach($extraclasses as $extraClassId)
        { 

            if (!$request->filled('is_default') || $request->is_default) {
                StudentExtraCurricularRecord::when(moduleStatusCheck('University'), function ($query) {
            ...
            if (generalSetting()->multiple_roll == 0 && $request->roll_number) {
                ...
                StudentExtraCurricularRecord::where('student_id', $request->student_id)
                ->where('school_id', auth()->user()->school_id)
            ...
            $StudentExtraCurricularRecord = new StudentExtraCurricularRecord;

            $StudentExtraCurricularRecord->student_id = $request->student_id;
            if ($request->roll_number) {
                $StudentExtraCurricularRecord->roll_no = $request->roll_number;
            }
            $StudentExtraCurricularRecord->is_promote = $request->is_promote ?? 0;
            $StudentExtraCurricularRecord->is_default = !$request->filled('is_default') || $request->is_default;

            $StudentExtraCurricularRecord->extra_class_id = $extraClassId;
            $StudentExtraCurricularRecord->session_id = $request->session;
            $StudentExtraCurricularRecord->school_id = Auth::user()->school_id;
            $StudentExtraCurricularRecord->academic_id = $request->session;
           
            $StudentExtraCurricularRecord->save();
           
            // if (moduleStatusCheck('University')) {
            //     $subjectIds = [];
            //     $this->assignSubjectStudent($StudentExtraCurricularRecord, $subjectIds, $pre_record);
            // }
            // if(directFees()){
            //     $this->assignDirectFees($StudentExtraCurricularRecord->id, $StudentExtraCurricularRecord->class_id, $StudentExtraCurricularRecord->section_id,null);
            // }
            ...
            try {
                ...
                if (!empty($data)) {
                    foreach ($data as $key => $value) {
                        ...
                        $ad_check = SmStudent::where('admission_no', $value->admission_number)->where('school_id', Auth::user()->school_id)->get();
                        //  return $ad_check;
                        ...
                        if ($ad_check->count() > 0) {
                                ...
                                if ($user) {
                                    if ($user->role_id == 2) {
                                        ...
                                        if (moduleStatusCheck('University')) {
                                            ...
                                            if(isset($user->student->id) && $user->student->id!='')
                                            {
                                                $studentRecord = StudentRecord::where('class_id', $request->class)
                                                    ->where('section_id', $request->section)
                                                    ->where('academic_id', $request->session)
                            ...
                            Toastr::error('Admission number should be unique('.$value->admission_number.')', 'Failed');
                            return redirect()->back();
                        }
                        ...
                        //         Toastr::error('Student Email address should be unique('.$value->email.')', 'Failed');
                        //         return redirect()->back();
                        //     }
                        // }
                        //Student Mobile unique check
                        if ($value->phone_number != "") {
                            $chk = DB::table('sm_students')->where('mobile', $value->phone_number)->where('school_id', Auth::user()->school_id)->count();
                            if ($chk >= 1) {
                                DB::rollback();
                                StudentBulkTemporary::where('user_id', Auth::user()->id)->delete();
                                Toastr::error('Student mobile should be unique.', 'Failed');
                        ...
                        if ($value->guardian_email != "") {
                            ...
                            if ($chk >= 1) {
                                DB::rollback();
                                StudentBulkTemporary::where('user_id', Auth::user()->id)->delete();
                                Toastr::error('Guardian Email address should be unique('.$value->guardian_email.')', 'Failed');
                                return redirect()->back();
                            }
                        }
                        ...
                        try {
                            ...
                            try {
                                $userIdParent = null;
                                $hasParent = null;
                                $mother_email = isset($value->mother_email) ? $value->mother_email : (isset($value->mothers_email) ? $value->mothers_email : null);
                                $mother_phone = isset($value->mother_phone) ? $value->mother_phone : (isset($value->mothers_phone) ? $value->mothers_phone : null);
                                if ($mother_email) {
                                    $user_parent = new User();
                                    $user_parent->role_id = 3;
                                    $user_parent->full_name = $value->mother_name ? $value->mother_name : $value->father_name;

                                    $data_parent['email'] = $mother_email;
                                    $user_parent->username = $mother_phone ? $mother_phone : $mother_email;
                                    $user_parent->email = $mother_email;
                                    $user_parent->phone_number = $mother_phone;
                                    $user_parent->password = Hash::make(123456);
                                    $user_parent->school_id = Auth::user()->school_id;
                                ...
                                try {
                                    if ($parentInfo) {
                                        ...
                                        $parent->father_education = $value->father_education;
                                        ...
                                        $parent->mother_education = $value->mother_education;
                                    ...
                                    try {
                                            ...
                                            $student->admission_date = $value->admission_date;
                                            ...
                                            $student->pin_code = $value->pin_code;

                                            $student->bloodgroup_id = $value->blood_group;
                                            $student->religion_id = $value->religion;
                                            $student->medium_of_instruction = $value->medium_of_instruction;
                                            
                                            $student->caste = $value->caste;

                                            $student->disability_group_name = $value->disability_group_name;
                                            $student->group_code = $value->group_code;
                                            //$student->mother_tounge = $value->mother_tounge;
                                            $student->mother_tounge = $value->mother_tounge;
                                            $student->medium = $value->medium;


                                            $student->save();

                                            //Create Section
                                            $existingSection = SmSection::where('section_name', $value->section_name)
                                                ->where('school_id', Auth::user()->school_id)
                                                ->first();
                                            ...
                                            if ($existingSection) {
                                                ...
                                                $section->created_by = auth()->user()->id; // Assuming this is the correct field name
                                            ...
                                            $existingClass = SmClass::where('class_name', $value->class_name)
                                                ->where('school_id', Auth::user()->school_id)
                                                ->first();
                                            ...
                                            if($existingClass) {
                                                ...
                                                $class = new SmClass();
                                                $class->class_name = $value->class_name;
                                                $class->created_at = YearCheck::getYear() . '-' . date('m-d h:i:s');
                                                $class->school_id = Auth::user()->school_id;
                                                $class->created_by = auth()->user()->id; // Assuming this is the correct field name
                                                $class->academic_id = !moduleStatusCheck('University') ? getAcademicId() : null;
                                            ...
                                            $checkClassSectionExist = SmClassSection::where('class_id', $class_id)
                                                ->where('section_id', $section_id)
                                                ->where('school_id', Auth::user()->school_id)
                                                ->first();

                                            if (empty($checkClassSectionExist)) {

                                                $smClassSection = new SmClassSection();
                                                $smClassSection->class_id = $class_id;
                                                $smClassSection->section_id = $section_id;
