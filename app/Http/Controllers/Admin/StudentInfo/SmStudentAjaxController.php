...
class SmStudentAjaxController extends Controller
{
    ...
    public function ajaxSelectStudent(Request $request)
    {
        ...
        $student_ids = $student_ids->toArray(); 
        $students = SmStudent::whereIn('id', $student_ids)->where('active_status', 1)->where('school_id', Auth::user()->school_id)->get()->map(function($student){
            return [
                'id' => $student->id,
                'full_name' => $student->first_name . ' ' . $student->last_name . '<span class="admission-no">' . $student->admission_no . '</span>',
    ...
    public function ajaxSiblingInfoUpdate(Request $request)
    {
        try {
            //Siblings
            if($request->relationship)
            {
                $res = $this->insertStudentSiblingRecord($request->merge([
    ...
    {
        $siblingExist = StudentSiblingRecord::where('sibling_id',$request->id)->where('student_id',$request->student_id)->first();
        if($siblingExist){
            return false;
        }else{
           $StudentSiblingRecord = new StudentSiblingRecord;
            $StudentSiblingRecord->student_id = $request->student_id;         
            $StudentSiblingRecord->sibling_class_id = $request->sibling_class;
            $StudentSiblingRecord->sibling_section_id = $request->sibling_section;
            $StudentSiblingRecord->sibling_id = $request->id;
            $StudentSiblingRecord->relationship = $request->relationship;
            $StudentSiblingRecord->school_id = Auth::user()->school_id;
            $StudentSiblingRecord->academic_id = $request->session;           
            $StudentSiblingRecord->save();
        }        
    }

    //Save Sibling details

    public function ajaxSiblingInfoSave(Request $request)
    {
        try {
            //Siblings
            if($request->relationship)
            {
                $res = $this->saveStudentSiblingRecord($request);
            }
            return response()->json([$res]);
            //return response()->json(['success']);
        } catch (\Exception $e) {
    ...
    //Siblings
    public function saveStudentSiblingRecord($request, $pre_record = null)
    {
        $siblingExist = StudentSiblingRecord::where('sibling_id',$request->id)->where('student_id',null)->first();
        if($siblingExist){
            return false;
        }else{
           $StudentSiblingRecord = new StudentSiblingRecord;
    ...
    public function ajaxLoadSiblingInfo(Request $request)
    {
        try {
                $student_id = $request->student_id;
                $data['siblings'] = SmStudent::select('sm_students.*')
                ->join('student_sibling_records','student_sibling_records.sibling_id','=','sm_students.id')
                ->where('student_sibling_records.student_id',$student_id)->get();

                $data['student_id'] = $student_id;
                $table_view = view('backEnd.studentInformation.student_siblings', $data)->render();
    ...
    public function ajaxClearSiblingInfo(Request $request)
    {
        try {
            StudentSiblingRecord::where('student_id',null)->delete();
    ...
    public function ajaxDeleteSibling(Request $request)
    {
        try {
            StudentSiblingRecord::where('student_id',$request->student_id)->where('sibling_id',$request->sibling_id)->delete();
            return response(["done"]);
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
