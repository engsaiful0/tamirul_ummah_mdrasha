        ...
        try {
            ...
            foreach ($request->routine as $key => $routine_data) {
                ...
                foreach ($days as $day) {
                    ...
                    //     ->where('school_id', Auth::user()->school_id)
                    //     ->first();
                    $new_start_time = date('H:i:s', strtotime(gv($routine_data, 'start_time')));
                    $new_end_time = date('H:i:s', strtotime(gv($routine_data, 'end_time')));
                    ...
                    ->where('academic_id', getAcademicId())
                    ->where('school_id', Auth::user()->school_id)
                    ->where(function($query) use ($new_start_time, $new_end_time) {
                        // Check if the new class start time is during an existing class
                        $query->where(function($q) use ($new_start_time, $new_end_time) {
                            $q->where('start_time', '<', $new_end_time)
                            ->where('end_time', '>', $new_start_time);
                        });
                    })
                    ->first();
