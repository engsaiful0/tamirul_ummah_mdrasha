...
class SmAssignSubjectController extends Controller
{
    ...
    {
        try {
            if((($request->subjects[0] == null) || ($request->teachers[0] == null))) {
            ...
            if ($request->update == 0) {
                ...
                if (isset($request->subjects)) {
                    foreach ($request->subjects as $key=>$subject) {
                        if ($subject != "" && !empty($request->teachers[$key])) {                            
                            if($request->section_id==null){
                               ...
                               $t_teacher=count($request->teachers);
                                ...
                                $exist_subject_create = SmAssignSubject::where('class_id',$request->class_id)
                                ->where('section_id', $request->section_id)
                                ->where('subject_id', $subject)
                                ->where('academic_id', getAcademicId())
                                ->where('school_id', Auth::user()->school_id)
                                ->first();
                                if (!$exist_subject_create) {
                                    $assign_subject = new SmAssignSubject();
                                    $assign_subject->class_id = $request->class_id;
                                    $assign_subject->school_id = Auth::user()->school_id;
                ...
                if($request->section_id ==null){
                    ...
                    if (! empty($request->subjects)) {
                        ...
                        foreach ($request->subjects as $key=>$subject) {
                            $k = 0;
                            if (!empty($subject) && !empty($request->teachers[$key])) {
                    ...
                    if (! empty($request->subjects)) {
                        ...
                        foreach ($request->subjects as $subject) {
                            ...
                            if (!empty($subject) && !empty($request->teachers[$i])) {
                                $exist_subject_create = SmAssignSubject::where('class_id',$request->class_id)
                                    ->where('section_id', $request->section_id)
                                    ->where('subject_id', $subject)
                                    ->where('academic_id', getAcademicId())
