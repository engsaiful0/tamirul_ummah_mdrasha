...
class SmExaminationController extends Controller
{
    ...
    {
        try {
            $studentId =  SmStudent::where('admission_no', $request->recordId)->value('id');
            $studentRecord = StudentRecord::where('student_id', $studentId)->first();
    ...
    public function previousStudentRecord(AdmissionNumberGetStudentRequest $request)
    {
        ...
        $allPromotedYearData = StudentRecord::with('class', 'section', 'academic')->where('student_id', $studentInfo->id)->get();
    ...
    public function exam_type_update(Request $request)
    {
        ...
        try {
            ...
            $update_exame_type->name_of_the_type = $request->name_of_examtype;
            ...
            try {
                if ($tables == null || $tables == '') {
                    ...
                    if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                        ...
                        if ($delete_query) {
                            Toastr::success('Exam Type deleted Successfully', 'Success');
                            return redirect()->back();
                        } else {
                            Toastr::error('Operation Failed', 'Failed');
                            return redirect()->back();
    ...
    public function percentMarkSheetReport(PercentMarkSheetReportRequest $request)
    {
        try {
            ...
            if ($exam_rule) {
                ...
                return view('backEnd.examination.report.marksheetReport', compact('exams', 'classes', 'mark_sheet', 'examInfo', 'subjectInfo', 'classInfo', 'sectionInfo', 'pass_mark', 'exam_rule', 'data','exam_code','exam_name'));
            } else {
                $mark_sheet = SmResultStore::query();
                $mark_sheet->where('exam_type_id', $request->exam_type);
                ...
                return view('backEnd.examination.report.marksheetReport', compact('exams', 'classes', 'mark_sheet','examInfo', 'subjectInfo', 'classInfo', 'sectionInfo','pass_mark','exam_rule','data','exam','exam_code','exam_name'));
            }
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
    ...
    public function percentMarksheetPrint(Request $request)
    {
        try {
            ...
            return view('backEnd.examination.report.marksheetReportPrint', compact('mark_sheet', 'examInfo', 'subjectInfo', 'classInfo', 'sectionInfo', 'pass_mark', 'exam_rule', 'data','exam_code','exam_name'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function topperStudent(){
        try {
            $classes = SmClass::where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get();
            return view('backEnd.examination.topper_student_view', compact('classes'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function toppersearch(Request $request)
    {
        // echo "Test";exit();
        try {
            $data = [];
            // $students = StudentExtraCurricularRecord::query();
                ...
                ->where('sm_assign_subjects.class_id', '=', $request->class_id)
                // ->where('sm_assign_subjects.section_id', '=', $datas['section_id'])
                ->where('sm_assign_subjects.academic_id', getAcademicId())
                ->where('sm_assign_subjects.school_id', Auth::user()->school_id)
                ->groupBy('sm_assign_subjects.subject_id')->get();
                // echo "<pre>";print_r($assignSubjects);exit();
                // dd(DB::getQueryLog());
                ...
                ->where('sm_exam_schedules.class_id', $request->class_id)
            ...
            $classes = SmClass::where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get();
            $data['classes'] = $classes;
            $data['assignSubjects'] = $assignSubjects;
            $data['current_exam'] = $current_exam;

            return view('backEnd.examination.topper_student_view', $data);
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
