...
use App\Http\Requests\Admin\Accounts\SmUpiAccountRequest;
...
class SmBankAccountController extends Controller
{
    ...
    public function upicreation()
    {
        // echo app(SmUpiAccount::class)->getTable();exit();
        try{
            return view('backEnd.accounts.upi_account');
        }catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function upistore(SmUpiAccountRequest $request)
    {
      try{
            $upi_account = new SmUpiAccount();
            $upi_account->upi_id = $request->upi_id;
            $upi_account->upi_name = $request->upi_name;
            $upi_account->note = $request->note;
            $upi_account->active_status = 1;
            $upi_account->academic_id = getAcademicId();
            $upi_account->school_id = Auth::user()->school_id;
            $upi_account->save();
    ...
    public function upiAccountDatatable()
    {
        try{
            $bank_accounts = SmUpiAccount::query();
            return DataTables::of($bank_accounts)
                    ->addIndexColumn()
                    ->addColumn('action', function ($row){
                        $btn = '<div class="dropdown CRM_dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right">'.
                                (userPermission('bank-account-delete') === true ? (Config::get('app.app_sync') ? '<span data-toggle="tooltip" title="Disabled For Demo"><a class="dropdown-item" href="#" >' . app('translator')->get('common.disable') . '</a></span>' :
                                '<a onclick="deleteUPIModal(' . $row->id . ');"  class="dropdown-item" href="#" data-toggle="modal" data-target="#deleteUPIAccountModal" data-id="' . $row->id . '"  >' . app('translator')->get('common.delete') . '</a>') : '') .
                            '</div>
                            </div>';
                        return $btn;
                    })
    ...
    public function destroy_upi(Request $request)
    {
        try{
            $tables = \App\tableList::getTableList('bank_id', $request->id);
            // print_r($tables);exit();
            try {
                // if ($tables==null) {
                    $bank_account = SmUpiAccount::destroy($request->id);
