...
class SmAccountsController extends Controller
{
    ...
    public function transactionManagement()
    {
        try {
            $payment_methods = SmPaymentMethhod::where('school_id', Auth::user()->school_id)->get();
            return view('backEnd.accounts.transaction_management', compact('payment_methods'));
        } catch (Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function accountsTransaction(Request $request)
    {
        try {
            $rangeArr = $request->date_range ? explode('-', $request->date_range) : "" . date('m/d/Y') . " - " . date('m/d/Y') . "";
            if ($request->date_range) {
            ...
            $search_info['method_id']='';
            if ($request->payment_method != "all") {
                $method_id = SmPaymentMethhod::find($request->payment_method);
                $search_info['method_id'] = $method_id->id;
            }

            $total_income = SmAddIncome::where('date', '<', $date_from)
                ->where('active_status', 1)
                ->where('school_id', Auth::user()->school_id)
                ->sum('amount');

            $total_expense = SmAddExpense::where('date', '<', $date_from)
                ->where('active_status', 1)
                ->where('school_id', Auth::user()->school_id)
                ->sum('amount');
            ...
            $externalVariable=$request->payment_method;
            $query1 = SmAddIncome::select('id','name','date','amount',DB::raw('"Income" as type'),'income_head_id','payment_method_id','account_id','created_at')
            ->where('date', '>=', $date_from)
            ->where('date', '<=', $date_to)
            ->where('active_status', 1)
             ->when($externalVariable !='all' , function ($query) use ($externalVariable) {
                    return $query->where('payment_method_id', '=', $externalVariable);
                })                
            ->where('school_id', Auth::user()->school_id);

            $query2 = SmAddExpense::select('id','name','date','amount',DB::raw('"Expense" as type'),'expense_head_id','payment_method_id','account_id','created_at')
            ->where('date', '>=', $date_from)
            ->where('date', '<=', $date_to)
            ->where('active_status', 1)
            ->when($externalVariable !='all' , function ($query) use ($externalVariable) {
            ...
            ->where('school_id', Auth::user()->school_id);
            $incomeExpenseResults = $query1->union($query2)
            ->orderBy('created_at') // Adjust the column based on your requirement
            ->get();
            return view('backEnd.accounts.transaction_management', compact('payment_methods','incomeExpenseResults','search_info','opening_balance'));
        } catch (Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
