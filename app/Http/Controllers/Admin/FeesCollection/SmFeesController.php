...
class SmFeesController extends Controller
{
    ...
    public function sendDuesExtraFeesEmail(Request $request){
        try{
            if(empty($request->student_list)) {
                Toastr::warning('Student Not Found', 'Warning');
                return redirect()->back();
            ...
            foreach($students as $student){
                ...
                $compact['due_amount'] = $due_amount[$i];
                $compact['fees_name'] = "";
                $compact['student_name'] = @$student->full_name;
    ...
    public function feesStatementSearch(Request $request)
    {
        ...
        try {
            ...
            if ($fees_assigneds->count() <= 0) {
                ...
                return redirect('fees-statement');
            } 
            else {
    ...
    public function feesPaymentInvoicePrint($id, $s_id)
    {
        ...
        try {
            ...
            if(moduleStatusCheck('University')){
                ...
                if($id=='all') {
                    $fees_assigneds = SmFeesAssign::where('record_id', $s_id)->get();
                } else {
                    foreach ($groups as $group) {
            ...
            $payment_info = SmFeesPayment::where('record_id', $s_id)->orderBy('id', 'desc')->first();
            $fees_created_by = '';
            $fees_by = '';
            if($payment_info && $payment_info->created_by)
            {
                $fees_created_by = User::find($payment_info->created_by);
                $fees_by = ($fees_created_by && isset($fees_created_by->full_name)) ? $fees_created_by->full_name : '-';
            }
            $transportPaymentInfo = SmTransportFeesPayment::select('route_id', DB::raw('SUM(amount) as total_amount'))
                ->where('record_id', $s_id)
                ->groupBy('route_id')
                ->first();
            //route_id
            $transport_total_amount = 0;
            $transport_balance_amount = 0;
            $routeInfo = null;
            if($transportPaymentInfo && $transportPaymentInfo->route_id) {
                $routeInfo = SmRoute::where('id', $transportPaymentInfo->route_id)->first();
                if ($routeInfo && $routeInfo->far) {
                    $transport_total_amount =  $routeInfo->far * 11;
                    $transport_balance_amount = $transport_total_amount - $transportPaymentInfo->total_amount;
                }
            }
            ...
            return view('backEnd.feesCollection.fees_payment_invoice_print')->with(['fees_assigneds' => $fees_assigneds, 'student' => $student,'unapplied_discount_amount'=>$unapplied_discount_amount, 'parent' => $parent,'id'=>$id, 'payment_info' => $payment_info, 'fees_created_by' => $fees_by, 'transport_total_amount' => $transport_total_amount,'transportPaymentInfo' => $transportPaymentInfo, 'routeInfo' => $routeInfo, 'transport_balance_amount' => $transport_balance_amount ]);
        } catch (\Exception $e) {
    ...
    public function transportFeesGenerateModal(Request $request, $amount, $student_id, $month_id, $month, $record_id, $route_id, $assigned_fee_amount)
    {
        try {
            $amount = $amount;
            $student_id = $student_id;
            ...
            return view('backEnd.feesCollection.transport_fees_generate_modal', compact('amount','month_id', 'month', 'student_id', 'data', 'method','banks','record_id','route_id','assigned_fee_amount'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function transportFeesPaymentStore(Request $request)
    {
        ...
        try {
            ...
            if($paidCount==0){
                $assigned_route_fees=$request->assigned_fee_amount;
            }
            $user = Auth::user();
            $fees_payment = new SmTransportFeesPayment();
            $fees_payment->student_id = $request->student_id;
            $fees_payment->month_id = $request->month_id;
            $fees_payment->month = $request->month;
            $fees_payment->amount = !empty($request->amount) ? $request->amount : 0;
            ...
            $fees_payment->school_id = Auth::user()->school_id;
            $fees_payment->slip = $fileName;
            $fees_payment->route_id = $request->route_id;
            $fees_payment->record_id = $request->record_id;
            $fees_payment->academic_id = getAcademicid();
            ...
            if ($result) {
                ...
                Toastr::error('Operation Failed', 'Failed');
                return Redirect::route('transport_fees_collect_student_wise', array('id' => $request->record_id));
            }
        } catch (\Exception $e) {
            // echo $e->getMessage();
            // exit;
    ...
    //Transport fees print
    public function transportFeesPaymentInvoicePrint($id, $s_id)
    {
        ...
        try {
            ...
            $fee_detail = [];
            foreach ($groups as $group) {
                $fee_detail[] = SmRoute::select('sm_routes.id','sm_routes.far','sm_routes.title','sm_transport_fees_payments.month','sm_transport_fees_payments.assigned_route_fees',DB::raw('SUM(sm_transport_fees_payments.amount) as total_amount'))
                ->leftjoin('sm_transport_fees_payments','sm_transport_fees_payments.route_id','=','sm_routes.id')
                ->where('sm_transport_fees_payments.record_id', $s_id)
                ->where('sm_transport_fees_payments.month_id', $group)
                ->where('sm_routes.active_status', 1)
                ->get(); 
            } 
            ...
            $payment_info = SmTransportFeesPayment::where('record_id', $s_id)->orderBy('id', 'desc')->first();
            ...
            return view('backEnd.feesCollection.transport_fees_payment_invoice_print')->with(['student' => $student, 'parent' => $parent,'id'=>$id, 'fee_detail' => $fee_detail, 'payment_info' => $payment_info, 'fees_created_by' => $fees_by]);
        } catch (\Exception $e) {
            Toastr::error('Operation Failed: ' . $e->getMessage(), 'Failed');
            return redirect()->back();
        }
