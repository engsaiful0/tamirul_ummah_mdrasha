...
class SmFeesMasterController extends Controller
{
    ...
    public function routeAssignDatatable(Request $request)
    {
        try{
            if ($request->ajax()) {
                $students = StudentRecord::query();
                ...
                $pre_assigned = StudentRecord::where('sm_students.route_list_id', $request->route_id)
                ->leftjoin('sm_students','sm_students.id','=','student_records.student_id')
                ->pluck('student_records.id')->toArray();
    ...
    public function getFeesTypes(Request $request)
    {
        $fees_types = SmFeesType::where('fees_group_id', $request->fees_group_id)->get();
        return response()->json($fees_types);
    }

    public function addFeesGroup(Request $request)
    {
        $fees_group = SmFeesGroup::firstorNew([
            'name' => $request->name,
            'school_id' => Auth::user()->school_id,
            'academic_id' => getAcademicId()
        ]);
        $fees_group->description = $request->name;
        $fees_group->save();

        $fees_groups = SmFeesGroup::get();
        return response()->json($fees_groups);
    }

    public function addFeesType(Request $request)
    {
        $fees_type = SmFeesType::firstorNew([
            'name' => $request->name,
            'fees_group_id' => $request->fees_group_id,
            'school_id' => Auth::user()->school_id,
            'academic_id' => getAcademicId()
