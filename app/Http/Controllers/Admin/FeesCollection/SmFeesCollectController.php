...
class SmFeesCollectController extends Controller
{
    ...
    //Transport Fees
    public function transportFees(Request $request)
    {
        try {
            $classes = SmClass::get();
            return view('backEnd.feesCollection.collect_transport_fees', compact('classes'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function transportSearch(SmFeesCollectSearchRequest $request)
    {
        try {
            $data = [];
            $students = StudentRecord::query();
            if(moduleStatusCheck('University')){
                $students =  universityFilter($students, $request);
                $students = $students->with('studentDetailTransport.parents')
                            ->whereHas('studentDetailTransport', function ($q){
                                $q->where('active_status', 1);
                            })->get();
            }else{
                if ($request->class != "") {
                ...
                if ($request->keyword != "") {
                    $students->whereHas('studentDetailTransport', function($q) use($request) {
                        $q->where('full_name', 'like', '%' . $request->keyword . '%')->orWhere('admission_no', $request->keyword)->orWhere('roll_no', $request->keyword)->orWhere('national_id_no', $request->keyword)->orWhere('local_id_no', $request->keyword);
                    });
                    
                }
                $students = $students->with('class','section','studentDetailTransport.parents')->whereHas('studentDetailTransport', function ($q)  {
                    $q->where('active_status', 1);
                })->get();
            }
            ...
            return view('backEnd.feesCollection.collect_transport_fees', $data);
        } catch (\Exception $e) {
            echo $e->getMessage();
            exit;
            Toastr::error('Operation Failed', 'Failed');
    ...
    //Collect Student Transport fees
    public function collectTransportFeesStudent(Request $request, $id)
    {
       try {
            $student = StudentRecord::with('studentDetailTransport','feesDiscounts')->find($id);
            $recordId=$id;
            $data['student'] = $student;
            ...
            return view('backEnd.feesCollection.transport_collect_fees_student_wise', $data);
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
