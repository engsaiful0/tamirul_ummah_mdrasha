...
use App\BankRejectedExtrafees;
use App\BankRejectedFees;
...
class SmFeesBankPaymentController extends Controller
{
    ...
    public function rejectExtraFeesPayment(SmRejectBankPaymentRequest $request){

        try{
                SmExtraCurricularFeesPayment::where('id',$request->id)->delete();
                $student = SmStudent::find($request->student_id);
                // print_r($student->email);exit();
                $rejectfees = new BankRejectedExtrafees();
                $rejectfees->extracurricular_record_id=$request->record_id;
                $rejectfees->amount=$request->reject_amount;
                $rejectfees->reason=$request->payment_reject_reason;
                $rejectfees->save();
                ...
                $student_record->paid_amount-=$request->reject_amount;
                ...
                // $view ="backEnd.feesCollection.bank_payment_reject_student";
                // $compact['data'] =  array( 
                //         'note' => $request->payment_reject_reason, 
                //         'date' =>date('Y-m-d'),
                //         'student_name' =>$student->full_name,
                // ); 
                $compact['note'] = $request->payment_reject_reason;
                $compact['student_name'] = $student->full_name;
                // $compact['amount'] = $request->reject_amount;
                $compact['date'] = date('Y-m-d');
                // print_r($compact);exit();
                send_mail($receiver_email, $receiver_name,'reject_bank_payment_student', $compact);
    ...
    public function rejectBankFeesPayment(SmRejectBankPaymentRequest $request){

        try{
                SmFeesPayment::where('id',$request->id)->delete();
                $student = SmStudent::find($request->student_id);
                // print_r($student->email);exit();
                $rejectfees = new BankRejectedFees();
                $rejectfees->extracurricular_record_id=$request->record_id;
                $rejectfees->amount=$request->reject_amount;
                $rejectfees->reason=$request->payment_reject_reason;
                $rejectfees->save();
