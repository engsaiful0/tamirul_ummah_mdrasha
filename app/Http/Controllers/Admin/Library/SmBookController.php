...
class SmBookController extends Controller
{
    ...
    public function issueBooks(Request $request, $member_type, $student_staff_id)
    {
        ...
        try {
            ...
            $totalIssuedBooks = SmBookIssue::where('school_id', Auth::user()->school_id)
                ->where('member_id', '=', $student_staff_id)
                ->whereHas('books')
                ->with('books')
                ->orderBy('id', 'DESC')
                ->get();
    ...
    {
        try {
            $books = SmBook::select('id', 'book_title')->where('school_id', Auth::user()->school_id)->get();
            $subjects = LibrarySubject::select('id', 'subject_name')->where('school_id', Auth::user()->school_id)->get();
            $now = Carbon::now();
            $issueBooks = SmBookIssue::where('school_id', Auth::user()->school_id)
                ->where('issue_status', 'I')
                ->whereHas('books')
                ->with('books', 'member')
                ->get();
    ...
    public function searchIssuedBook(Request $request)
    {
        try {
            ...
            $query = SmBookIssue::where('school_id', Auth::user()->school_id)
                ->where('issue_status', 'I')
                ->whereHas('books');
            ...
            if ($request->book_number) {
                $query->whereHas('books', function ($q) use ($request) {
                    $q->where('book_number', $request->book_number);
                });
            }
           
            if ($request->subject_id) {
                $query->whereHas('books', function ($q) use ($request) {
                    $q->where('book_subject_id', $request->subject_id);
                });
            }
            
            $issueBooks = $query->with('books', 'member')->get();
            
            $books = SmBook::select('id', 'book_title')->where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
            $subjects = LibrarySubject::select('id', 'subject_name')->where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
    ...
    public function importBook()
    {
        try {
            ...
            return view('backEnd.library.import_book');
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function bookBulkStore(Request $request)
    {
        ...
        if ($file_type <> 'csv' && $file_type <> 'xlsx' && $file_type <> 'xls') {
            ...
            try {
                ...
                Excel::import(new BookImport, $request->file('file'), 's3', \Maatwebsite\Excel\Excel::XLSX);
                // DB::commit();
                ...
                $data = BookBulkTemporary::where('user_id', Auth::user()->id)->get();
                    ...
                    foreach ($data as $key => $value) {
                        if(isSubscriptionEnabled()){
                            ...
                            // if(\Modules\Saas\Entities\SmPackagePlan::student_limit() <= $active_student && saasDomain() != 'school'){
                                ...
                                DB::commit();
                                BookBulkTemporary::where('user_id', Auth::user()->id)->delete();
                                Toastr::error('Your student limit has been crossed.', 'Failed');
                        ...
                        try {
                            ...
                            $books->book_title = $value->book_title;
                            $books->book_category_id = $value->book_category_id;
                            $books->book_number = $value->book_number;
                            $books->isbn_no = $value->isbn_no;
                            $books->publisher_name = $value->publisher_name;
                            $books->author_name = $value->author_name;
                            if (@$value->book_subject_id) {
                                $books->book_subject_id = $value->book_subject_id;
                            }
                            $books->rack_number = $value->rack_number;
                            if (@$value->quantity != "") {
                                $books->quantity = $value->quantity;
                            }
                            if (@$value->book_price != "") {
                                $books->book_price = $value->book_price;
                            }
                            $books->details = $value->details;
                            $books->post_date = date('Y-m-d');
                            $books->created_by = auth::user()->id;
                            $books->school_id = Auth::user()->school_id;
                            if(moduleStatusCheck('University')){
