    ...
    public function store(EventRequest $request)
    {
        try {
            ...
            $data = [
                ...
                'event_description' => $request->event_des,
            ...
            // Map for_whom to role names and get user IDs
            $user_ids = [];
            $role_names = null;
            ...
            if ($request->for_whom == 'All') {
                ...
                $role_names = null; // null means all roles
            ...
            // Send notifications via email/SMS/web/app if notification setting exists
            if (!empty($user_ids)) {
                try {
                    $this->sent_notifications('New_Event', $user_ids, $data, $role_names);
                } catch (\Exception $e) {
            ...
            // Also save web notifications (for backward compatibility)
            if ($request->for_whom == 'All') {
                $users = User::where('school_id', Auth::user()->school_id)->where('active_status', 1)->get();
