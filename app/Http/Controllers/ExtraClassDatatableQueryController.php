...
class ExtraClassDatatableQueryController extends Controller
{
    public function studentDetailsDatatable(Request $request)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $classes = SmExtraCurricularClass::where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->withoutGlobalScope(StatusAcademicSchoolScope::class)->get();
            ...
            return view('backEnd.studentInformation.extraCurricular.student_details', compact('classes', 'class_id', 'name', 'roll_no', 'sessions', 'section', 'academic_year','data'));
        }
        if ($request->ajax()) {
            $records = StudentExtraCurricularRecord::query();
            ...
            ->when(moduleStatusCheck('University') && $request->filled('un_faculty_id'), function ($u_query) use ($request) {
                ...
                $query->when($request->class, function ($query) use ($request) {
                    $query->where('extra_class_id', $request->class);
                });
            })
           ...
           $all_students =  SmStudent::with('studentExtraCurricularRecords','studentExtraCurricularRecords.class')->whereIn('id',$student_records)
            ...
            $students = SmStudent::with(['gender', 'studentExtraCurricularRecords' => function ($q) use ($request) {
            ...
            // return $request;
            $students = $students->with('studentExtraCurricularRecords','studentExtraCurricularRecords.class')->where('sm_students.school_id', Auth::user()->school_id)
                ->with(array('parents' => function ($query) {
                    $query->select('id', 'fathers_name');
                }))
                ->with(array('gender' => function ($query) {
            ...
            return Datatables::of($all_students)
                ...
                ->addColumn('semester_label', function ($row) use ($request) {
                    $semester_label=[];
                    foreach ($row->studentExtraCurricularRecords as $label) {
                ...
                ->addColumn('class_sec', function ($row) use ($request) {
                    $class_sec=[];
                    foreach ($row->studentExtraCurricularRecords as $classSec) {
                        if (moduleStatusCheck('University')) {
        ...
        return view('backEnd.studentInformation.extraCurricular.students');
    }

    public function searchStudentList(Request $request)
    {
        $student_ids = StudentExtraCurricularRecord::when($request->academic_year, function ($query) use ($request) {
            $query->where('academic_id', $request->academic_year);
        })
    ...
    public function unAssignStudentList(Request $request){
        if ($request->ajax()) {
            $all_students = SmStudent::with('parents','gender','category')->wheredoesnthave('studentExtraCurricularRecords')->where('school_id', Auth::user()->school_id)->where('academic_id',getAcademicId())->get();
            return Datatables::of($all_students)
                ->addIndexColumn()
                ->addColumn('dob', function ($row) {
                    $dob = dateConvert(@$row->date_of_birth);
    ...
    public function disableStudentList(Request $request){
        ...
        if ($request->ajax()) {
            ...
            $student_ids = StudentExtraCurricularRecord::when($request->class_id, function ($query) use ($request) {
                                $query->where('extra_class_id', $request->class_id);
                            })
          ...
          $all_students = $students->with('studentExtraCurricularRecords.class','parents','gender','category')->where('active_status', 0)->where('school_id', Auth::user()->school_id)->get();
    ...
    public function ajaxExpenseList(Request $request){
        ...
        public function studentTransportReportAjax(Request $request){
            if($request->ajax()){
                $students = SmStudent::with('StudentExtraCurricularRecord.class','parents','route','vehicle','drivers')->whereHas('vehicle')->get();
                return Datatables::of($students)
                ->addIndexColumn()
