        ...
        try {
           ...
           // DB::statement('SET FOREIGN_KEY_CHECKS=0;');
            ...
            if ($id != 2 && $id != 3) {
                ...
                ->whereNotInDeaActiveModulePermission()
                ->where('menu_status', 1)
                ->when(generalSetting()->fees_status == 1, function($q) use ($id) {
                    // Allow fees_collection permissions for accountant (role 6) and admin (role 5) even when Fees module is enabled
                    if (!in_array($id, [5, 6])) {
