    ...
    <section class="sms-breadcrumb mb-40 white-box">
        ...
        <div class="row">
                ...
                <div class="d-flex align-items-center">
                    <a href="{{ url('/sync-student-permissions') }}" class="btn btn-primary btn-sm mr-2" onclick="return confirm('This will sync all student permissions from file to database. Continue?');">
                        <i class="fa fa-sync"></i> Sync Permissions
                    </a>
                    <div class="bc-pages">
                    ...
                    <div class="single_permission" id="{{ $permission->id }}">
                        ...
                        <div class="permission_header d-flex align-items-center justify-content-between">
                            ...
                            <div>
                                ...
                                <label
                                    for="Main_Module_{{ $permission->id }}">@if($permission->lang_name){{ __($permission->lang_name) }}@else{{ $permission->name }}@endif</label>
                            </div>
                                ...
                                @foreach ($permission->subModule as $row2)
                                    <li>
                                        <div class="submodule">
                                            ...
                                            <label
                                                for="Sub_Module_{{ $row2->id }}">@if($row2->lang_name){{ __($row2->lang_name) }}@else{{ $row2->name }}@endif
                                            </label>
                                            <br>
                                        </div>
                                        <br>
                                            ...
                                            @foreach ($row2->subModule as $row3)
                                                <li>
                                                    <div class="module_link_option_div" id="{{ $row2->id }}">
                                                        ...
                                                        <label
                                                            for="Option_{{ $row3->id }}">@if($row3->lang_name){{ __($row3->lang_name) }}@else{{ $row3->name }}@endif
                                                        </label>
                                                        <br>
                                                    </div>
                                                </li>
