        ...
        return Datatables::of($walletAmounts)
            ...
            ->addColumn('t_status', function ($row) {
                ...
                elseif ($row->status == "refund") {
                    $btn = '<button class="primary-btn small bg-primary text-white border-0">' . app('translator')->get('wallet.refund') . '</button>';
                }
                else {
        ...
        if($request->ajax()){
            ...
            ->addColumn('t_status', function ($row) {
                if ($row->status == "pending") {
                    ...
                    $btn = '<button class="primary-btn small bg-success text-white border-0">' . app('translator')->get('wallet.approved') . '</button>';
                } else {
            ...
            if ($request->ajax()) {
                ...
                return DataTables::of($walletAmounts)
                        ...
                        ->addColumn('user_name', function($row) {
                            return ['full_name' => $row->user_name];
                        })
                        ...
                        ->addColumn('walletStatus', function($row){
                            ...
                            if ($row->status == 'pending'){
                                ...
                                $btn = '<button class="primary-btn small bg-success text-white border-0" style="cursor:default">' . __('wallet.approved') . '</button>';
                            } else{
                                $btn = '<button class="primary-btn small bg-danger text-white border-0" style="cursor:default">' . __('common.rejected') . '</button>';
                            }
                            return $btn;
                        })
