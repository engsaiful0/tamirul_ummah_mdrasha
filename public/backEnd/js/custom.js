(function($) {
    ...
    // Sibling section info for student admission
    $(document).ready(function() {
        $("#classSelectStudentSiblings").on("change", function() {
          ...
          $("#sectionSelectStudentSiblings").empty().append(
            $("<option>", {
                value:  '',
                text: window.jsLang('select_section') + ' *',
            })
        ...
        if (!class_id){
            $("#sectionSelectStudentSiblings").trigger('change').niceSelect('update');
            return;
        }
            var formData = {
                id: $(this).val(),
            ...
            $.ajax({
                ...
                beforeSend: function() {
                    $('#select_section_loader_siblings').addClass('pre_loader');
                    $('#select_section_loader_siblings').removeClass('loader');
                },
                success: function(data) {
                    ...
                    $.each(data, function(i, item) {
                        ...
                        if (item.length) {
                            $.each(item, function(i, section) {
                                $("#sectionSelectStudentSiblings").append(
                                    $("<option>", {
                                        value: section.id,
                                        text: section.section_name,
                                    })
                ...
                complete: function() {
                    i--;
                    if (i <= 0) {
                        $('#select_section_loader_siblings').removeClass('pre_loader');
                        $('#select_section_loader_siblings').addClass('loader');
                    }
                }
            });
        });
        ...
        /********Load Students******/
        $("#sectionSelectStudentSiblings").on("change", function() {
            var url = $("#url").val();
            var i = 0;

          var class_id = $(this).val();

          $("#studentSiblings").empty().append(
            $("<option>", {
                value:  '',
                text: window.jsLang('select_section') + ' *',
            })
        ...
        if (!class_id){
            $("#studentSiblings").trigger('change').niceSelect('update');
            return;
        }
            var formData = {
                section: $(this).val(),
                class: $("#classSelectStudentSiblings").val(),
            };
            // get section for student
            $.ajax({
                type: "GET",
                ...
                beforeSend: function() {
                    $('#select_student_loader_siblings').addClass('pre_loader');
                    $('#select_student_loader_siblings').removeClass('loader');
                },
                success: function(data) {
                ...
                complete: function() {
                    i--;
                    if (i <= 0) {
                        $('#select_student_loader_siblings').removeClass('pre_loader');
                        $('#select_student_loader_siblings').addClass('loader');
                    }
                }
            });
        });
        ...
        $("#studentSiblings").on("change", function() {
            if($(this).val()){
    ...
    $(document).ready(function() {
        ...
        $("#SelectDormitoryEdit").on("change", function() {
            var url = $("#url").val();
            var i = 0;
            ...
            $.ajax({
                ...
                url: url + "/" + "ajaxRoomDetails",
                beforeSend: function() {
                    $('#select_dormitory_loader_edit').addClass('pre_loader');
                    $('#select_dormitory_loader_edit').removeClass('loader');
                },
                success: function(data) {
                    $.each(data, function(i, item) {
                        if (item.length) {
                ...
                complete: function() {
                    i--;
                    if (i <= 0) {
                        $('#select_dormitory_loader_edit').removeClass('pre_loader');
                        $('#select_dormitory_loader_edit').addClass('loader');
                    }
                }
            });
        });
    ...
    $(document).ready(function() {
        $(".relationButton").on("click", function() {
            if ($(this).val() == "F") {
                ...
                if($("#fathers_email").length){
                    $("#guardians_email").val($("#fathers_email").val());
                }
                if($("#parent_address").length){
                    $("#guardians_address").val($("#parent_address").val());
                }
                ...
                if($("#mothers_email").length){
                    $("#guardians_email").val($("#mothers_email").val());
                }
                if($("#parent_address").length){
                    $("#guardians_address").val($("#parent_address").val());
                }
    ...
    var fileInputRoute = document.getElementById("upload_route_file");
    if (fileInputRoute) {
        
        fileInputRoute.addEventListener("change", showFileName);

        function showFileName(event) {
            var fileInputRoute = event.srcElement;
            var fileName = fileInputRoute.files[0].name;
            document.getElementById("placeholderPhoto").placeholder = fileName;
        }
    }
...
// Promote Student
    ...
    $('#select_section_fees').change(function () {
        $('#search_student').submit();
    });
    ...
    validateFormFees = () => {
        ...
        if (amount < 0) {
            ...
            document.getElementById("amount_error").innerHTML =
                "Deposit amount can not be greater than remaining";
            return false;
    ...
    validateAddNewroutine = () => {
        ...
        $("tr[id^='row_']").each(function () {
            ...
            var start_time = document.getElementById(`start_time_${rowId}`);
            var end_time = document.getElementById(`end_time_${rowId}`);
            var subject = document.getElementById(`subject_${rowId}`);
            var room = document.getElementById(`room_${rowId}`);
            var teacher = document.getElementById(`teacher_${rowId}`);
            ...
            const startDateTimeString = `${month}/${day}/${year} ${start_time.value}`;
            const endDateTimeString = `${month}/${day}/${year} ${end_time.value}`;

            const startDateTimeInSeconds = new Date(startDateTimeString).getTime() / 1000; // Convert to seconds
            const endDateTimeInSeconds = new Date(endDateTimeString).getTime() / 1000; 

            if (start_time && start_time.value === "") {
                $(`#start_time_${rowId}`).siblings(".start_time_error").text("Start Time field is required");
                i++;
            } else {
                $(`#start_time_${rowId}`).siblings(".start_time_error").text("");
            }
    
            if (end_time && end_time.value === "") {
                $(`#end_time_${rowId}`).siblings(".end_time_error").text("End Time field is required");
                i++;
            } else {
                $(`#end_time_${rowId}`).siblings(".end_time_error").text("");
            }
    
            if (start_time && end_time && start_time.value && end_time.value) {
                if (endDateTimeInSeconds <= startDateTimeInSeconds) {
                    $(`#end_time_${rowId}`).siblings(".end_time_error").text("End Time must be greater than Start Time");
                    i++;
                } else {
                    $(`#end_time_${rowId}`).siblings(".end_time_error").text("");
                }
            ...
            if (subject && subject.value === "") {
                $(`#subject_${rowId}`).siblings(".text-danger").text("Subject field is required");
                i++;
            } else {
                $(`#subject_${rowId}`).siblings(".text-danger").text("");
            }
    
            if (room && room.value === "") {
                $(`#room_${rowId}`).siblings(".text-danger").text("Room field is required");
                i++;
            } else {
                $(`#room_${rowId}`).siblings(".text-danger").text("");
            }
    
            if (teacher && teacher.value === "") {
                $(`#teacher_${rowId}`).siblings(".text-danger").text("Teacher field is required");
                i++;
            } else {
                $(`#teacher_${rowId}`).siblings(".text-danger").text("");
            }
        });
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    subject_teacher +=
                        ...
                        divCount +
                        ")' type='button'>";
                    subject_teacher += "<span class='ti ti-trash' ></span>";
        ...
        $("#addNewRoom").on("click", function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    appendRow +=
                        "<span class='ti ti-trash text-white' onclick='deleteExamRow(" +
                        rowCount +
                        ")'></span>";
                    appendRow += "</button>";
                    appendRow += "</td>";
    ...
    $(document).on(
        "click",
        "#extra_curricular_fees_invoice_button",
        function(event) {
            var url = $("#url").val();
            var student_id = $("#student_id").val();
            //console.log(student_id);
            ...
            if (sList != "") {
                $("#extra_curricular_fees_invoice_button").attr(
                    "href",
                    url + "/extraclass-fees-payment-invoice-print/" + sList + "/" + student_id
                );
                $("#extra_curricular_fees_invoice_button").attr("target", "_blank");
            } else {
                toastr.error("Please select fees!", "Error");
            }
        }
    ...
    $(document).on(
        "click",
        "#transport_fees_groups_invoice_print_button",
        function(event) {
            var url = $("#url").val();
            var student_id = $("#student_id").val();
            //console.log(student_id);
            ...
            if (sList != "") {
                $("#transport_fees_groups_invoice_print_button").attr(
                    "href",
                    url + "/transport-fees-payment-invoice-print/" + sList + "/" + student_id
                );
                $("#transport_fees_groups_invoice_print_button").attr("target", "_blank");
            } else {
                toastr.error("Please select fees!", "Error");
            }
        }
    ...
    $(document).on(
        "click",
        "#quick_transport_fees_groups_invoice_print_button",
        function(event) {
            var url = $("#url").val();
            var student_id = $("#student_record_id").val();
            var sList = $('#print_ids').val();
            if (sList != "") {
                $("#quick_transport_fees_groups_invoice_print_button").attr(
                    "href",
                    url + "/transport-fees-payment-invoice-print/" + sList + "/" + student_id
                );
                $("#quick_transport_fees_groups_invoice_print_button").attr("target", "_blank");
            } else {
                toastr.error("Please select fees!", "Error");
            }
        }
    ...
    $(document).on(
        "click",
        "#quick_fees_groups_invoice_print_button",
        function(event) {
            var url = $("#url").val();
            var student_id = $("#student_record_id").val();
            //console.log(student_id);
            var sList = "all";
            if (sList != "") {
                $("#quick_fees_groups_invoice_print_button").attr(
                    "href",
                    url + "/fees-payment-invoice-print/" + sList + "/" + student_id
                );
                $("#quick_fees_groups_invoice_print_button").attr("target", "_blank");
            } else {
                toastr.error("Please select fees!", "Error");
            }
        }
    ...
    changePassword = (id, role) => {
        // Validate that ID exists and is not null/empty
        if (!id || id === null || id === 'null' || id === '' || id === 'undefined') {
            toastr.error("User ID is missing. Cannot reset password.", "Error Alert", {
                timeOut: 5000,
            });
            return false;
        }
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data.op === false || data.status === 'error') {
                    toastr.error(data.msg || data.message || "Operation Failed!", "Error Alert", {
                        timeOut: 5000,
                    });
                } else {
                    setTimeout(function() {
                        toastr.success(
                            data.msg || "Success! Password has been reset as default 123456",
                            "Success Alert", {
                                iconClass: "customer-info",
                            }, {
                                timeOut: 2000,
            ...
            error: function(xhr, status, error) {
                ...
                setTimeout(function() {
                    toastr.error(errorMessage, "Error Alert", {
                        timeOut: 5000,
                    });
                }, 500);
            },
    ...
    (function() {
        $(document).ready(function() {
            $(".switch-input").on("change", function() {
                // Get ID from tr element's id attribute or from hidden input
                var trId = $(this).parents("tr").attr("id");
                var hiddenId = $(this).closest("tr").find("input[type='hidden'][id='id']").val();
                var id = trId || hiddenId;
                
                // Validate that ID exists
                if (!id || id === '' || id === 'undefined') {
                    toastr.error("User ID is missing. Please refresh the page and try again.", "Error Alert", {
                        timeOut: 5000,
                    });
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.status === 'error') {
                            toastr.error(data.message || "Operation Failed!", "Error Alert", {
                                timeOut: 5000,
                            });
                        } else {
                            setTimeout(function() {
    ...
    (function() {
        $(document).ready(function() {
            $(".parent-login-disable").on("change", function() {
                ...
                // Validate that ID exists
                if (!id || id === '' || id === 'undefined') {
                    toastr.error("Parent User ID is missing. Please refresh the page and try again.", "Error Alert", {
                        timeOut: 5000,
                    });
                    // Revert checkbox state
                    $(this).prop('checked', !$(this).prop('checked'));
    ...
    // 100 Percent Mark Start
        ...
        function loadsiblings(student_id,url){
            var formData = {
                student_id: student_id,
            };
            $.ajax({
                ...
                data: formData,
                dataType: "json",
                url: url + "/" + "ajaxLoadSiblingInfo",
                success: function(data) {
                    $('#loadSiblingInfo').html(data.table_view);
                },
                error: function(data) {
                    // console.log("Error:", data);
                },
        ...
        // update student sibling 
        $(document).ready(function() {
            var url = $("#url").val();
            var student_id = $("#student_id").val();
            loadsiblings(student_id,url);        
            $("#update_sibling_button").on("click", function() {
                var select_sibling_name = $("#select_sibling_name").val();
                var relationship = $("#relationship").val();
                var class_id = $("#select_sibling_class").val();
                var section_id = $("#select_sibling_section").val();
                var relationship = $("#relationship").val();
                var student_id = $("#student_id").val();
                ...
                $.ajax({
                    ...
                    data: formData,
                    dataType: "json",
                    url: url + "/" + "ajaxSiblingInfoUpdate",
                    success: function(data) {
                        if(data=='false'){
                            setTimeout(function() {
                            toastr.error("Sibling Info Exists.!", "Error Alert", {
                                timeOut: 5000,
                                });
                            }, 500);
                            return false;
            ...
            /** Add more Sibling **/
            clearNewSiblings(student_id,url);
            $("#save_sibling_button").on("click", function() {
                var select_sibling_name = $("#select_sibling_name").val();
                var relationship = $("#relationship").val();
                var class_id = $("#select_sibling_class").val();
                var section_id = $("#select_sibling_section").val();
            ...
            function clearNewSiblings(){
                $.ajax({
                    type: "GET",
                    //data: formData,
                    dataType: "json",
                    url: url + "/" + "ajaxClearSiblingInfo",
                    success: function(data) {
                        $('#loadSiblingInfo').html(data.table_view);
                    },
                    error: function(data) {
...
function removeStudentSibling(sibling_id){
    $('#sibling_id').val(sibling_id);
    $('#removeSiblingModal').show();
}
...
function deleteSibling(){
    ...
    $.ajax({
        ...
        data: formData,
        dataType: "json",
        url: url + "/" + "ajaxDeleteSibling",
        success: function(data) {
            $('#removeSiblingModal').hide();
            loadStudentsiblings(student_id);
        },
        error: function(data) {
            // console.log("Error:", data);
        },
...
function loadStudentsiblings(student_id){
    var formData = {
        student_id: student_id,
    };
    var url = $("#url").val();
...
function validateAddExtraactivities() {

    var url = $("#url").val();
    var academic=$('#common_academic_year').val();
    var student=$('#common_select_student').val();
    var select_class=$('#common_select_class').val();
    var section=$('#common_select_section').val();

    var i = 0;        

    if (academic == "") {
        document.getElementById("academic_error").innerHTML =
...
function validateExtraFilters() {
    var extraCurricularClass=$('#select_extra_class').val();
    var i = 0; 
    if (extraCurricularClass == "") {
        document.getElementById("extra_class_error").innerHTML = "Extra curricular class is required";
        i++;
    } else {
        document.getElementById("extra_class_error").innerHTML = "";
    }
    if (i > 0) {
        return false;
    }
...
function validatePayrollEarningsSettings() {
    var earnings_name=$('#earnings_name').val();
    var earnings_type_name=$('#earnings_type_name').val();
    var i = 0; 
    if (earnings_name == "") {
        document.getElementById("group_error_earnings").innerHTML = "Group field is required";
        i++;
    } else {
        document.getElementById("group_error_earnings").innerHTML = "";
    }
    if (earnings_name == "") {
        document.getElementById("group_error_deductions").innerHTML = "Group field is required";
        i++;
    } else {
        document.getElementById("group_error_deductions").innerHTML = "";
    }
    if (earnings_name == "") {
        document.getElementById("name_error_earnings").innerHTML = "Name field is required";
        i++;
    } else {
        document.getElementById("name_error_earnings").innerHTML = "";
    }    
    if (earnings_type_name == "") {
        document.getElementById("type_name_error_earnings").innerHTML = "Percentage field is required";
        i++;
    } else {
        document.getElementById("type_name_error_earnings").innerHTML = "";
    }
    if (i > 0) {
        return false;
    }
...
function validatePayrollDeductionSettings() {
    var deductions_name=$('#deductions_name').val();
    var deductions_type_name=$('#deductions_type_name').val();
    var i = 0; 

    if (deductions_name == "") {
        document.getElementById("name_error_deductions").innerHTML = "Name field is required";
        i++;
    } else {
        document.getElementById("name_error_deductions").innerHTML = "";
    }
    if (deductions_type_name == "") {
        document.getElementById("type_name_error_deductions").innerHTML = "Percentage field is required";
        i++;
    } else {
        document.getElementById("type_name_error_deductions").innerHTML = "";
    }
    if (i > 0) {
        return false;
    }
...
$(document).ready(function() {
    $('#add-earning-btn').click(function(){
        //$('#add-earnings').toggle('slow');
        $('#add-earning-btn').hide();
        $('#add-earnings').show();
        $('#earnings_group_name').val('');
        $('#earnings_type_name').val('');
        $('#earnings_name').val('');
        $('.payroll-settings-btn').html('Save');        
    });
    $('#earning-cancel-btn').click(function(){
        $('#add-earnings').hide();
        $('#add-earning-btn').show();
        $('#earning_id').val('');
    });
    $('#deduction-cancel-btn').click(function(){
        $('#add-deductions').hide();
        $('#add-deduction-btn').show();
        $('#earning_id').val('');
    });

    $('#add-deduction-btn').click(function(){
        $('#add-deduction-btn').hide();
        $('#add-deductions').show();   
        $('#deductions_group_name').val('');
        $('#deductions_type_name').val('');
        $('#deductions_name').val('');
        $('.payroll-settings-btn').html('Save');       
    });
    ...
    $('#one-tab').click(function(){
        ...
        $('#add-deductions').hide();
        $('#add-earning-btn').show();
        $('#onloadpayroll').val(false);
        //$("#load_payment_Settings").html('');
        /*****Load Deduction Data*****/
        loadpayroll_earnings();
    ...
    $('#two-tab').click(function(){
        ...
        $.ajax({
                type: "GET",
                data: formData,
                url: "ajaxPayrollDeduction",
                success: function(data) {
                    $("#load_payment_Settings").html(data);
                },
                error: function(data) {
                    console.log("Error:", data);
                },
    ...
    $('#three-tab').click(function(){
        ...
        var onloadpayroll = $('#onloadpayroll').val();
        if(onloadpayroll=='true')
        {
            $('#onloadpayroll').val(false);
            loadpayroll_earnings();
        }
        ...
        $('#schoolbtnsubmit').on('click',function()
        {
            $(this).html('Please wait ...')
                .attr('disabled','disabled');
        ...
        $("#billing-form").validate({
            rules: {
                ...
                "paid[]": {
                    ...
                    max: function(element) {
                        // Get the corresponding amount value
                        var amount = parseFloat($(element).closest("tr").find("input[name='amount[]']").val());
                        return amount;
                    }
                }
            },
            messages: {
                ...
                "paid[]": {
                    ...
                    number: "Please enter a valid number.",
                    max: "Paid amount cannot be greater than the amount."
                }
            },
            errorPlacement: function(error, element) {
                // Display error messages under each input field
                error.appendTo(element.closest("tr").find(".error"));
            }
        });
...
function loadpayroll_earnings(){
    var formData='';
    $.ajax({
            type: "GET",
            data: formData,
            url: "ajaxPayrollEarnings",
            success: function(data) {
                $("#load_payment_Settings").html(data);
            },
            error: function(data) {
...
function earningModal(id,amount,reason,remarks){
    $('#earnings_staff_id').val(id);
    $('#addEarningModal').modal('show');
    $('#amount').val(amount);
    $('#remarks').val(remarks);
    //$("#reason option[value='Bonus']").prop("selected", true);
}
function deductionModal(id,amount,reason,remarks){
    $('#deductions_staff_id').val(id);
    $('#addDeductionModal').modal('show');
    $('#deduction_amount').val(amount);
    $('#deduction_remarks').val(remarks);
    //$("#deduction_reason option[value="+reason+"]").prop("selected", true);
}


function validateEarnings() {

    var percentages = document.querySelectorAll('.percentage-input');
    var percenage_type = document.querySelectorAll('.percentage-type');
    var total = 0;
    var basic_percent = 0
    //alert(percenage_type[0].value);
    for (var i = 0; i < percentages.length; i++) {

        var basic = percentages[0].value/100;
        var hra = percentages[1].value/100;
        if(percentages[0].value<percentages[1].value){
            var hraErrorElement = document.getElementById('hraPercentError');
            hraErrorElement.textContent = 'HRA percentage must be less than ('+percentages[0].value+')%';
            return false;
        }

        if(percenage_type[i].value!='% of Basic'){
             total += parseInt(percentages[i].value) || 0;
        }else{
            var basic = percentages[0].value/100;
            var hra = percentages[1].value/100;
            basic_percent = basic * hra * 100;
            total += parseInt(basic_percent) || 0;
        }
    }

    var errorElement = document.getElementById('percentageError');
    
    
    if (total !== 100) {
        errorElement.textContent = 'Exclude HRA Total percentage must be 100%. ('+total+')';
        return false;
    } else {
...
function earningPercentage(id,group_id){
    if(id==2){
        var percentages = document.querySelectorAll('.percentage-input');
        var percenage_type = document.querySelectorAll('.percentage-type');
        var total = 0;
        ...
        var hra = percentages[1].value/100;
        basic_percent = basic * hra * 100;
        total += parseInt(percentages[0].value) || 0;
        total += parseInt(basic_percent) || 0;
        all_earnings = 100 - total;
        $('#earning_percentage_1').html('('+total+'%)');        
        $('#earning_percentage_2').html('('+all_earnings+'%)');        
    }
}

function downloadPayslipModal(id){
    $('#emp_id').val(id);
...
function validateFeeType() {
    var assign = true;
    var checkedCheckbox = $('#studentTable .student-selected:checked');
    if (checkedCheckbox.length > 0) {
        // Get the closest parent <tr> element's ID
        var checkedValues = checkedCheckbox.map(function() {
            var closestSelect = $(this).closest('tr').find('select');
                // Get the value of the selected option
                if(closestSelect.val()==''){
                    assign = false;
                    $(this).closest('tr').find('span').html('Fee type is required!');
                    return false;
                }
        }).get();
    }
...
function save_ctc_format(val){
     var url = $("#url").val();
     $.ajax({
        url: '{{ route(update.ctc) }}',
        url: url + "/" + "updateCtcMonth",
        method: 'POST',
        data: {
            ctc_month: val,
...
function validateFeesDetails() {
    ...
    var url = $("#url").val();
    var fees_category=$('#select_fees_category').val();
    var student=$('#common_select_student').val();
    var select_class=$('#common_select_class').val();
    var section=$('#common_select_section').val();
    ...
    if (fees_category == "") {
        document.getElementById("fees_category_error").innerHTML =
            "Fees category is required";
        i++;
    } else {
        document.getElementById("fees_category_error").innerHTML = "";
    }
...
$(document).on('blur', 'input[name="discount[]"]', function() {
    ...
    var paidInput = $row.find('input[name="paid[]"]');
    var discount = parseFloat($(this).val());
    var paid = parseFloat(paidInput.val());
    var balanceInput = $row.find('input[name="balance_amount[]"]');
    var balance = parseFloat(balanceInput.val());

    // Subtract the discount from the paid amount
    paid -= discount;

    // If the discount is greater than the paid amount, set paid amount to 0
    if (discount > paid) {
        paid = 0.00;
    }
    ...
    // Perform validation for paid amount against balance amount
    if (paid > balance) {
        paidInput.after('<span class="error-message">Paid cannot be greater than balance amount.</span>');
        paidInput.val(0.00);
    }
});

$(document).on('blur', 'input[name="paid[]"]', function() {
    var $row = $(this).closest('tr');
    var discount = parseFloat($row.find('input[name="discount[]"]').val());
    var balance = parseFloat($row.find('input[name="balance_amount[]"]').val());
    var paid = parseFloat($(this).val());

    if (discount < balance) {
        // Enable the paid amount input field if discount is less than balance
        $(this).prop('disabled', false);
    }


    if (paid > (balance - discount)) {
        // If paid amount is greater than the difference between balance and discount, reset it
        
        var amount = balance - discount;
        if(amount<0) {
            $(this).val('0.00');
        } else {
            $(this).val((balance - discount).toFixed(2));
        }        
    }
});
...
function validatePayForm() {
    $('.error-message').hide();
    var hasPaidGreaterThanZero = false;
    var isValid = true;

    $('table#table_id_table tbody tr').each(function(index) {
        var amount = parseFloat($(this).find('input[name="amount[]"]').val());
        var paid = parseFloat($(this).find('input[name="paid[]"]').val());
        var balance_amount = parseFloat($(this).find('input[name="balance_amount[]"]').val());
        var discount = parseFloat($(this).find('input[name="discount[]"]').val());

        if (paid > balance_amount) {
            $(this).find('input[name="paid[]"]').after('<span class="error-message">Paid cannot be greater than balance amount.</span>');
            isValid = false;
            $(this).find('input[name="paid[]"]').val(0.00);
        }

        if (paid > 0 || discount > 0) {
            hasPaidGreaterThanZero = true;
        }
    });

    if (!hasPaidGreaterThanZero) {
        $('#common-error-message').show();
        isValid = false;
    }
...
function validateTransportPayForm() {
    var checkedCount = $('.fees-groups-print:checked').length;
    if(checkedCount==0) {
        toastr.error("Please select transport fees for any one month!", 'Opps!');
        return false;
    } else {
        return;
    }
...
$(document).ready(function() {
    ...
    $('input[name="fee_type"]').change(function() {
        // Hide all fee type divs
        $('.fee-type').hide();
        
        // Show the selected fee type div
        $('#' + $(this).val() + '-fees').show();
    });
});
    ...
    document.addEventListener('DOMContentLoaded', (event) => {
        const modal = document.getElementById('transportFeeModal');
        const closeButton = document.getElementsByClassName('close')[0];
        const monthInput = document.getElementById('hid_month');
        const studentIdInput = document.getElementById('hid_student_id');
        const recordIdInput = document.getElementById('hid_record_id');
        const monthValueInput = document.getElementById('hid_month_value');
        const routeIdInput = document.getElementById('hid_route_id');
        const statusIdInput = document.getElementById('hid_status_id');
        const transportFeeInput = document.getElementById('hid_transport_fee');
        //var statusId = 1;

        document.querySelectorAll('.transport_fees_status').forEach(selectElement => {
            selectElement.addEventListener('change', (event) => {
                const selectedValue = event.target.value;
                const index = event.target.getAttribute('data-index');
                const studentId = event.target.getAttribute('data-student-id');
                const recordId = event.target.getAttribute('data-record-id');
                const monthValue = event.target.getAttribute('data-month-value');
                const routeId = event.target.getAttribute('data-route-id');
                const transportFeeAmount = event.target.getAttribute('data-transport-fee');
                
                monthInput.value = index;
                studentIdInput.value = studentId;
                recordIdInput.value = recordId;
                monthValueInput.value = monthValue;
                routeIdInput.value = routeId;
                statusIdInput.value = selectedValue;
                transportFeeInput.value = transportFeeAmount;
        ...
        closeButton.addEventListener('click', () => {
            ...
            document.querySelectorAll('.transport_fees_status').forEach(selectElement => {
                if (selectElement.value === 'inactive') {
                    selectElement.value = 'active'; // Reset the dropdown to Active
                }
            });
        });
